...
```mermaid
graph LR
    App[正規アプリ] -- 申請 --> Att[アテステーション プロバイダ]
    Att -- 発行 --> Token((App Check トークン))
    Token -- 提示 --> Firebase[Firebase サービス]
```
...
```mermaid
flowchart LR
  A["正規アプリ📱<br>(App Checkトークン付き)"] -->|✅ OKどうぞ| B["Firebase"]
  C["それ以外😈<br>(トークン無し/変なトークン)"] -.->|❌ 拒否| B
```
...
```mermaid
flowchart LR
  U["ユーザー👤"] -->|ログイン| Auth["Firebase Authentication<br>（誰か？）"]
  Auth --> Rules["Security Rules<br>（何ができるか？）🛡️"]
  App["クライアントアプリ📱"] -->|通信の出所| AppCheck["App Check<br>（正規アプリか？）🧿"]
```

つまり、App Checkは「Rulesの代わり」じゃなくて、**別レイヤーで入口を守る担当**って感じだよ〜🔒✨

```mermaid
graph TD
    A[AI 連打] -- 高額請求 --> Error[破産😱]
    B[Storage 大量Up] -- ストレージ圧迫 --> Error
    C[Firestore 乱読] -- 課金・負荷 --> Error
```
...
```mermaid
graph LR
    S1[SDK 導入] --> S2[メトリクス監視]
    S2 --> S3{問題なし?}
    S3 -- YES --> S4[Enforce 強制有効]
```
...
sequenceDiagram
    ...
    participant F as Other Firebase Services

    A ->> AC: initializeAppCheck
    Note over A,F: 以降の通信にトークンが自動で載る
    A ->> F: Firestore / Storage 呼び出し
```
