...
```mermaid
sequenceDiagram
  participant App as ブラウザ(React)
  participant AC as App Check / Prove
  participant FB as Firebase各サービス

  App->>AC: 証明プロバイダで判定
  AC-->>App: トークン発行
  App->>FB: リクエストにトークンを乗せて通信
```
...
```mermaid
graph LR
    User["リクエスト"] -- 未検証 --> S["App Check スタンプ台"]
    S -- ポンッ！ --> Valid["検証済みトークン付き"]
    Valid -- 入場 --> FB["Firebase 各サービス"]
```
...
graph TD
    subgraph Browser ["Client (React)"]
        SDK["Firebase SDK"] -- 自動付与 --> R1["Firebase リクエスト"]
        Code["カスタムコード"] -- 手動付与 (Header) --> R2["自前 API"]
    end
    subgraph Server ["Backend"]
        R1 --> FB["Firebase Services"]
        R2 -- X-Firebase-AppCheck --> API["Custom API"]
    end
```
...
```mermaid
flowchart TD
  A["1. App Check 実装"] --> B["2. メトリクス監視👀"]
  B --> C{"問題なし？"}
  C -- "Yes" --> D["3. 強制(Enforcement) ON🔒"]
  C -- "No" --> E["原因究明と修正へ"]
```
...
```mermaid
graph LR
    subgraph Short ["短く設定 (30m)"]
        S1["安全↑"] --- S2["負荷↑"]
    end
    subgraph Long ["長く設定 (7d)"]
        L1["安全↓"] --- L2["性能↑"]
    end
```
