...
```mermaid
graph LR
    0.0["0.0 (Bot 🤖)"] --- 0.5["0.5 (Default)"] --- 1.0["1.0 (Human 👤)"]
    style 0.0 fill:#f99
    style 1.0 fill:#9f9
```
...
```mermaid
graph TD
    S1[Score: 0.8] -- Pass --> T{Threshold: 0.5}
    S2[Score: 0.2] -- Block --> T
    T --> OK[Verified]
    T --> NG[Access Denied]
```
...
```mermaid
flowchart LR
  A["しきい値: 高(0.9)"] --> B["安全↑ / UX事故リスク↑"]
  C["しきい値: 低(0.3)"] --> D["UX↑ / 乱用リスク↑"]
```
...
```mermaid
graph TD
    Start[0.5 で開始] --> Monitor[メトリクス監視]
    Monitor --> Adjust[実トラフィック見て調整]
```
...
```mermaid
graph LR
    App[Web App] --- Settings[reCAPTCHA v3 設定]
    Settings --- Slider["App risk threshold (スライダー)"]
```
...
```mermaid
flowchart TD
  O["ユーザー操作"] --> R["reCAPTCHA v3 スコア(0.0〜1.0)"]
  R --> C{"App Check しきい値と比較"}
  C -->|スコア >= しきい値| OK["✅ OK（正規っぽい）"]
  C -->|スコア < しきい値| NG["❌ NG（拒否される）"]
```
