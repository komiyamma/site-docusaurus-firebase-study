...
```mermaid
flowchart LR
  A["正規アプリ(React)📱"] -->|App Check通行証🎫| FB["Firebase 各サービス"]
  FB -->|✅ 許可| A
  Evil["不正スクリプト😈"] -.->|通行証なし/偽造| FB
  FB -.->|❌ 拒否| Evil
```
...
```mermaid
graph TD
    subgraph firebase_ts ["firebase.ts (Central Hub)"]
        App[Firebase App]
        AC[App Check] --> App
        FS[Firestore] --> App
        ST[Storage] --> App
    end
    main[main.tsx] -- Import --> firebase_ts
```
...
sequenceDiagram
    ...
    B ->> M: Load
    M ->> F: Import / Initialize
    F ->> F: App Check Start
    M ->> A: Render App
    A ->> F: Call Firestore (with Token)
```
...
```mermaid
graph TD
    P1[二重初期化 ❌] --- S1[Global Guard]
    P2[タイミングミス ❌] --- S2[main.tsx 先頭 import]
    P3[更新忘れ ❌] --- S3[AutoRefresh: true]
    P4[開発環境 ❌] --- S4[Debug Provider]
```
...
```mermaid
graph LR
    Project[Vite Project] -- npm i --> SDK[firebase SDK]
    SDK --- AC[App Check]
    SDK --- FS[Firestore]
    SDK --- AI[AI Logic]
```
...
graph LR
    ...
    subgraph Safety ["Abuse Prevention"]
        AC[App Check 🧿]
        RL[Rate Limit 💸]
    end
    subgraph Backend ["Firebase AI Logic"]
        GenAI[Generative AI]
    end
    Btn --> AC --> RL --> GenAI
```
