...
flowchart LR
  subgraph Current["現状"]
    v3["reCAPTCHA v3<br>(スコア非公開/シンプル)"]
  end
  subgraph Future["将来の移行先"]
    Ent["reCAPTCHA Enterprise<br>(スコア細分化/運用特化)"]
  end
  Current -->|公式推奨| Future
```
...
```mermaid
flowchart TD
  Config["環境変数等からのsite key取得"] --> Factory["Provider Factory<br>🏭 createProvider()"]
  Factory -->|v3 または Enterprise| Provider["Providerインスタンス"]
  Provider --> Init["initializeAppCheck() ⚡<br>※アプリ内で1回のみ"]
```
...
```mermaid
graph TD
    subgraph Inside ["appCheck.ts (Encapsulated)"]
        createProvider --> initAppCheck
        initAppCheck --> Instance[App Check Instance]
    end
    main[main.tsx] -- import & init --> Inside
    Other[Other Services] -- use --> Instance
```
...
```mermaid
graph TD
    Env[Environment Variable] --> Choice{Choose Provider}
    JSON[Public JSON] --> Choice
    Choice -- "kind: v3" --> v3[v3 Provider]
    Choice -- "kind: enterprise" --> Ent[Enterprise Provider]
```
...
sequenceDiagram
    ...
    App ->> AC: getToken (silent)
    AC -->> App: token
    App ->> API: Request + X-Firebase-AppCheck
    API ->> API: Verify with Admin SDK
    API -->> App: response
```
...
```mermaid
graph TD
    Trap[Token Expired Error!]
    Trap --- C1{Auto Refresh ON?}
    C1 -- NO --> Dead[時限爆弾 💣]
    C1 -- YES --> Live[安定稼働 🔁]
```
...
```mermaid
graph LR
    AI[AI Auditor] -- Scan --> Code[Codebase]
    Code -- Detect --> E1[Double Init]
    Code -- Detect --> E2[Timing Gap]
    Code -- Detect --> E3[Refresh Lack]
```
