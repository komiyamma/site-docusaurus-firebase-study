...
```mermaid
flowchart TD
  A["1. ✅ App Check SDK 導入"] --> B["2. 👀 メトリクス監視<br>(影響確認)"]
  B --> C{"安全に強制ONできるか？"}
  C -->|Yes| D["3. 🎛️ 強制ON (Enforcement)"]
  C -->|No| E["原因調査と修正"]
  E --> B
```
...
```mermaid
pie title App Check リクエストの分類
    "✅ Verified (正常)" : 70
    "🧓 Outdated client (古いSDK等)" : 15
    "🕵️ Unknown origin (出所不明)" : 10
    "❌ Invalid (不正トークン)" : 5
```
...
トークンはあるが **無効**。
偽装クライアント、あるいはエミュレート環境由来的…など。([Firebase][1])
...
graph TD
    M[メトリクスを確認]
    M --> C1{Verified が大半?}
    C1 -- Yes --> C2{Outdated は少?}
    C1 -- No --> Wait[改善・周知]
    C2 -- Yes --> GO[✅ Go Enforcement]
    C2 -- No --> Wait
```
...
```mermaid
graph TD
    subgraph StepA ["Step A: 操作"]
        A1[Firestore 操作]
        A2[Storage 操作]
        A3[AI Logic 操作]
    end
    subgraph StepB ["Step B: 観察"]
        B1[メトリクス画面]
    end
    StepA -->|Request発生| StepB
```
...
```mermaid
graph TD
    Mission[AI Audit Mission]
    Mission --- T1[ Verified率 メモ]
    Mission --- T2[異常原因切り分け]
    Mission --- T3[強制順序 計画]
```
