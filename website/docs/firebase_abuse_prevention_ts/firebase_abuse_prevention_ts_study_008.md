...
```mermaid
graph TD
    T1["大量読み取り 🔓"] -- 情報漏洩 --> Risk["リスク"]
    T2["大量書き込み 🧨"] -- スパム・課金 --> Risk
    T3["不正データ投入 🧪"] -- データ破壊 --> Risk
    T4["AI 踏み台 🤖"] -- 高額請求 --> Risk
```
...
flowchart LR
  ...
  App -- "トークンあり🎫" --> Check{"App Check<br>(入口の扉🚪)"}
  Evil -- "トークンなし/偽造✖" --> Check

  Check -- "✅ 許可" --> Rules{"Firestore Rules<br> (権限/内容チェック🛡️)"}
  Check -. "❌ 拒否" .-> Block["アクセス遮断🚫"]

  Rules -- "ルール適合📝" --> DB["(#quot;Firestore🗃️#quot;)"]
  Rules -. "違反(例:他人のメモ)" .-> Block2["アクセス遮断🚫"]
```
...
```mermaid
graph LR
    UI["Components"] -- use --> Repo["memoRepo.ts"]
    Repo -- addMemo / listLatest --> SDK["Firestore SDK"]
    SDK --> DB["(#quot;Firestore#quot;)"]
```
...
```mermaid
graph TD
    Req["Incoming Request"] --> Match{"match path?"}
    Match -- Yes --> Auth{"signedIn?"}
    Auth -- Yes --> Data{"validData?"}
    Data -- Yes --> Allow["✅ Allow"]
    Match -- No --> Deny["❌ Deny"]
    Auth -- No --> Deny
    Data -- No --> Deny
```
...
```mermaid
graph TD
    Client["Web/Mobile"] -- Request --> Rules{"Security Rules"}
    Rules -- Allow --> DB["(#quot;Firestore#quot;)"]
    Admin["Admin SDK / Functions"] -- Bypass --> DB
```
...
graph LR
    ...
    User --> AC --> Rules --> Rate --> AI["Generative AI"]
```
