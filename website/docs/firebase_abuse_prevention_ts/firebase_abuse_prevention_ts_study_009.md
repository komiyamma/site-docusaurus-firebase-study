...
```mermaid
graph TD
    T1["巨大ファイル 📦"] -- 高額保管料 --> Loss["被害"]
    T2["連打 UP 🔁"] -- 帯域消費 --> Loss
    T3["直リンク拡散 🌍"] -- 転送量増 --> Loss
    T4["拡張子偽装 🧨"] -- セキュリティリスク --> Loss
    T5["不適切画像 🧑‍🎤"] -- 規約違反 --> Loss
```
...
```mermaid
flowchart TD
  Req["画像アップロード要求📱"] --> AC{"🧿 App Check<br>(正規アプリか？)"}
  AC -- "✅ 通過" --> Rules{"🧾 Storage Rules<br>(ログイン済/サイズ/MIME)"}
  AC -- "❌ 拒否" --> Block1["ブロック🚫"]
  
  Rules -- "✅ 通過" --> Storage["(#quot;Cloud Storage📦#quot;)"]
  Rules -- "❌ 拒否" --> Block2["ブロック🚫"]
  
  Storage -- "トリガー⚡" --> Backend{"🧰 バックエンド処理<br>(AI検査/サムネ生成)"}
```
...
```mermaid
graph TD
    Req["Upload"] --> UID{"uid 一致?"}
    UID -- Yes --> Size{"< 2MB?"}
    Size -- Yes --> MIME{"image/.*?"}
    MIME -- Yes --> Allow["✅ Allow"]
```
...
* `VITE_ENABLE_APP_CHECK=false` のとき：![Upload UI](./picture/firebase_abuse_prevention_ts_study_009_03_upload_ui.png)

```mermaid
graph TD
    subgraph UI ["Upload Component"]
        Picker["ファイル選択"]
        Bar["プログレスバー 🚀"]
        Preview["プレビュー 📷"]
    end
...
```mermaid
graph TD
    URL["getDownloadURL"] -- 公開URL --> Risk["誰でも見れる ⚠️"]
    SDK["getBlob/getBytes"] -- SDK経由 --> Rules["Rulesで認可 🛡️"]
```
