...
flowchart TD
  ...
  subgraph Server["Cloud Functions ☁️"]
    Callable["Callable関数<br>(enforceAppCheck: true 🧿)"]
    Auth["Auth/管理者判定🧑‍💼"]
    Logic["重要ロジック実行処理🔥"]
  end
  
  Req -- "App Check等自動付与🎫" --> Callable
  Callable -- "✅ 通過" --> Auth
  Auth -- "✅ OK" --> Logic
```
...
```mermaid
flowchart TD
  A["1. Callable作成 ☎️"] --> B["2. enforceAppCheck: true 🧿"]
  B --> C{"3. ログイン(Auth)必須👤"}
  C -- "Yes" --> D{"4. 管理者(Admin)か？🧑‍💼"}
  D -- "Yes" --> E["5. 削除実行❌📝"]
  C -- "No" --> Err1["エラー: 未ログイン"]
  D -- "No" --> Err2["エラー: 権限不足"]
```
...
* Callable はクライアント SDK が **App Check トークン等を自動で付ける**ので、クライアント側でヘッダを手で付ける必要は基本ありません👍

```mermaid
graph TD
    A["Who: Auth 🔑"] --- Goal(("Cloud Functions 🏯"))
    B["Which App: App Check 🧿"] --- Goal
    C["Permissions: Admin 🧑‍💼"] --- Goal
```
...
sequenceDiagram
    ...
    App ->> SDK: call adminDeleteMemo()
    SDK ->> AC: getToken()
    AC -->> SDK: token🎫
    SDK ->> Func: Request + token🎫
    Func -->> App: response
```
...
> App Check 強制後は「通らない呼び出しは拒否」が基本になるので、UIが無言で落ちない作りが超重要です🙂🧯
...
graph LR
    User --> AC["App Check"]
    AC --> Auth["Admin Auth"]
    Auth --> Rate["AI Rate Limit"]
    Rate --> AI["Generative AI"]
```
...
```mermaid
graph LR
    AI["AI Assistant"] -- Plan --> Code["Secure Functions"]
    AI -- Review --> Code
    AI -- Test --> Code
```
