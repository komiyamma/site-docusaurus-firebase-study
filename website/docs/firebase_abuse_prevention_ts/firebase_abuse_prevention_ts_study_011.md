...
```mermaid
graph LR
    User[正規ユーザー] -- 1. トーク取得🎫 --> Attacker[攻撃者👿]
    Attacker -- 2. 1回目送る🎫 --> API[API]
    Attacker -- 3. 2回目送る(リプレイ)🎫 --> API
    API -- 全員OK (通常のApp Check) --> Attacker
```
...
```mermaid
graph LR
    C[Client] -- "limited-use🎫" --> S[Server]
    S -- "Verify & Consume ☑️" --> AC[App Check Service]
    AC -- "OK (1st time)" --> S
    S -- "Success" --> C
```
...
```mermaid
graph TD
    subgraph S1 [Security UP]
        R1[Anti-Replay ON]
    end
    subgraph S2 [Speed DOWN]
        R2[1 往復 追加 🐢]
    end
    S1 --- S2
```
...
```mermaid
graph TD
    subgraph High ["高価値/高コスト (ON候補)"]
        AI[AI整形ボタン 🤖]
        Admin[管理者操作 🧑‍💼]
    end
    subgraph Low ["高頻度 (OFF推奨)"]
        List[一覧取得 📥]
        Stat[ログ送信 📊]
    end
```
...
graph LR
    ...
    Config --- E[enforceAppCheck: true]
    Config --- C[consumeAppCheckToken: true ♻️]
```
...
```mermaid
graph LR
    Client[React App] -- request --> SDK[httpsCallable]
    SDK --- Opt[limitedUseAppCheckTokens: true 🧾]
```
...
sequenceDiagram
    ...
    participant Server as サーバ☁️ (consume: true)
    
    Client->>Server: limited-use トークン付きで要求🎫
    Server-->>Server: トークン検証 ＆ 消費 (consume) ☑️
    Server->>Client: 処理結果(OK) ✅
    
    Note over Client,Server: 攻撃者がトークンを再利用しようとする
    Client->>Server: 同じトークンで再要求🎫
    Server-->>Server: 「既に消費済み」判定
    Server->>Client: 拒否 (もう使ったよ) 🚫
```
