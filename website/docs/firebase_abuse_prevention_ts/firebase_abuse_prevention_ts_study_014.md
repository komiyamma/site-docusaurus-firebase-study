...
```mermaid
flowchart LR
  Console["1. Firebase Console<br>(トークン登録)"] -->|トークン発行| GitHub["2. GitHub Secrets🔐"]
  GitHub -->|環境変数として注入| CI["3. CIランナー🏃‍♂️"]
  CI -->|Debug Provider ON| Firebase["バックエンド検証✅"]
```
...
```mermaid
graph LR
    C[Console] -- Copy 🎫 --> G[GitHub Settings]
    G -- "New Secret" --> S[APP_CHECK_DEBUG_TOKEN]
```
...
```mermaid
graph TD
    T1[1. Set Token to self 🎫] --> T2[2. Import SDK 📦]
    T2 --> T3[3. Initialize App Check 🧿]
```
...
> Debug Provider は「デバッグトークンが入ってるとき、SDKがデバッグ扱いにできる」仕組みなので、**コードの形は保ったまま** ![CI vs App Check](./picture/firebase_abuse_prevention_ts_study_014_01_ci_failure.png)

```mermaid
graph LR
    Local[Local Dev] -- "Manual Debug Token" --> OK[✅ Pass]
    CIRunner[CI Runner] -- "No Token" --> Fail[❌ 403 Forbidden]
    CIRunner -- "Secret Token" --> OK
```
...
sequenceDiagram
    ...
    Runner ->> Build: npm run build (with Secret🎫)
    Build ->> Server: npm run preview
    Server -->> Test: Wait for Port 4173
    Test ->> Server: Run E2E (App Check Enabled)
```
...
```mermaid
graph TD
    PR[Pull Request] -- Dummy AI --> Fast[高速/低コスト ⚡]
    Main[Main Merge] -- Real AI --> Verify[確実な検証 ✅]
    Night[Nightly Build] -- Heavy Tests --> Quality[品質担保 🌙]
```
...
```mermaid
graph TD
    S1[Save Memo] --> S2[Click AI Polish]
    S2 --> S3[Verify Result]
    S3 -- Success --> OK[App Check Pass ✅]
```
