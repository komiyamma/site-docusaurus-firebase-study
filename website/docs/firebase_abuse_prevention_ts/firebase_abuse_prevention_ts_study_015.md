...
```mermaid
graph TD
    T1[A. App Check 取得失敗 🧿💥] -- "reCAPTCHA 等" --> UI
    T2[B. バックエンド 拒否 🚫🧱] -- "enforcement ON" --> UI
    T3[C. AI 回数制限 🤖⏳] -- "Rate Limit" --> UI
    UI[UXでの分類と表示]
```
...
```mermaid
flowchart TD
  RawError["生のエラー (FirebaseError等)"] --> Normalize{"エラー分類器<br>(normalizeError)"}
  Normalize -->|appCheck/throttled| E1["1️⃣ 安全確認一時停止⏳<br>(再読み込み/待機)"]
  Normalize -->|permission-denied| E2["2️⃣ アクセス拒否🚫<br>(再読み込み/サポート)"]
  Normalize -->|rate_limited| E3["3️⃣ 混雑中🤖⏳<br>(待機)"]
  Normalize -->|network| E4["4️⃣ 通信不安定📡<br>(再試行)"]
```
...
graph TD
    fn[実行関数] --> Try{try}
    Try -- Success --> OK[そのまま返却 ✅]
    Try -- Catch --> Norm[normalizeError 🗂️]
    Norm --> UI[状態反映 🧯]
```
...
```mermaid
graph TD
    AI[AI要求] --> AppCheck{App Check?}
    AppCheck -- Fail --> Msg1[安全確認エラー 🧿🚫]
    AppCheck -- OK --> Rate{Rate Limit?}
    Rate -- Fail --> Msg2[混雑エラー 💸⏳]
```
