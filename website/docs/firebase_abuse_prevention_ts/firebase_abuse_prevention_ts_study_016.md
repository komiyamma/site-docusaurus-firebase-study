...
```mermaid
graph LR
    User[ユーザー] -- Request --> AC{App Check Switch}
    AC -- OFF (監察) --> OK[✅ 全員通過]
    AC -- ON (強制) --> Filter{トークン確認}
    Filter -- Valid --> Pass[✅ 通過]
    Filter -- Invalid/None --> Deny[❌ 拒否]
```
...
graph TD
    M[App Check Metrics]
    M --> V[Verified ✅: 安全]
    M --> O[Outdated client 🧓: 古いアプリ]
    M --> U[Unknown origin 👻: 怪しい]
    M --> I[Invalid ❌: 不正]
```
...
```mermaid
graph LR
    C[Console GUI] -- Click --> B["[Enforce] Button 🧿"]
    B --> W["15 min sync ⏳"]
    W --> E[Enforced!]
```
...
```mermaid
graph TD
    Request --> Gate{AI Logic Gate 🚪}
    Gate -- Check 1: App Check --> Q{Check 2: Quota}
    Q -- Check OK --> AI[Gemini API]
    Gate -- Fail --> Error[❌ Blocked]
```
...
```mermaid
flowchart TD
  A["観測期間<br>(メトリクス確認)"] --> B["Day1: Firestore強制ON🗃️"]
  B --> C["Day2: Storage強制ON📦"]
  C --> D["Day3: AI Logic強制ON🤖"]
  
  B -.->|エラー確認| Z{"ロールバック判定🧯"}
  C -.->|エラー確認| Z
  D -.->|エラー確認| Z
  Z -->|NG| R["一時的 unenforce"]
```
...
```mermaid
graph LR
    Alert[炎上発生! 🔥] --> Judge{Rollback?}
    Judge -- YES --> Switch[Unenforce 🎛️]
    Switch -- Restore --> Peace[沈火 🧯]
```
...
```mermaid
graph LR
    Agent[AI Agent / Antigravity] -- MCP protocol --> MCP[Firebase MCP Server]
    MCP -- API calls --> Firebase[Your Firebase Project]
```
