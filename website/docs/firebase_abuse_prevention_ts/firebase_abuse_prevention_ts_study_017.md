...
flowchart TD
  Q{"どの言語で書く？"}
  Q -- "Node.js/TS" --> N["王道・最新エコシステム✅<br>(リプレイ対策も〇)"]
  Q -- "Python" --> P["可能だが Preview ⚠️<br>(リプレイ対策は非対応)"]
  Q -- ".NET" --> D["Cloud Run functionsで実装🧩<br>(Firebaseと連携)"]
```
...
timeline
    section Lifecycle
    2024 : Node 18 (Deprecated)
    2025 : Node 20 (Soon Deprecated)
    2026 : Node 22 (Current / Stable) : Node 24 (Cutting Edge)
```
...
```mermaid
graph TD
    subgraph Support [Python 3.10-3.13]
        AppCheck["App Check Enforce (Preview ⚠️)"]
    end
    subgraph NotSupport [Not Supported]
        Replay["Anti-Replay (consume) 🚫"]
    end
```
...
```mermaid
graph LR
    JS["Node/JS"] -- "First Class Support" --> Genkit["Firebase Genkit"]
    PY["Python"] -- "Local ONLY (No Deploy)" --> Genkit
```
...
```mermaid
graph LR
    R["Runtime: Node 22/24"] -- "The Environment" --> App
    S["SDK: firebase-functions 6.2+"] -- "The Tools" --> App
```
...
```mermaid
graph TD
    P["package.json"] -- "engines: {node}" --> Node["Node Runtime"]
    F["firebase.json"] -- "runtime: python313" --> Python["Python Runtime"]
```
...
```mermaid
graph LR
    Agent["AI Agent"] -- "MCP Tool: check_runtime" --> MCP["Firebase MCP"]
    MCP -- "Read Metadata" --> Proj["Firebase Project"]
```
