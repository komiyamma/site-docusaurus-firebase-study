...
```mermaid
sequenceDiagram
    participant App as React("ブラウザ")📱
    participant API as 自前API("Express等")🧱
    participant FB as Firebase Admin SDK🛡️
    
    App->>App: トークン取得🎫
    App->>API: fetch (X-Firebase-AppCheck: トークン)
    API->>FB: verifyToken("トークン")
    
    alt 検証成功 ✅
        FB-->>API: decodedToken (claims)
        API-->>App: 結果を返す (200 OK)
    else 検証失敗 ❌
        FB-->>API: エラー
        API-->>App: 拒否 (401 Unauthorized)
    end
```
...
```mermaid
graph TD
    Safe["Safe: HTTP Header ✅"] -- X-Firebase-AppCheck --> API
    Unsafe["Unsafe: URL Query 🚫"] -- "?token=xxxxx" --> API
    API["自前API"]
```
...
```mermaid
graph LR
    App["React App"] -- "headers: { 'X-Firebase-AppCheck': token }" --> API["fetch('/api/.../')"]
```
...
```mermaid
graph TD
    Req["Any Request"] --> Mid{"App Check Middleware 🛡️"}
    Mid -- "Token Valid ✅" --> Next["Next Handler"]
    Mid -- "Token None/Invalid ❌" --> Deny["401 Unauthorized"]
```
...
graph LR
    ...
    PK["X-Firebase-AppCheck"] -- "どのアプリ? (App Check)" --> API
    AK["Authorization"] -- "誰? (Auth)" --> API
    API["Secure API"]
```
...
graph LR
    T["Token 🎫"] --> V{"Verify"}
    V -- "consume: true" --> S["App Check Service"]
    S -- "1st Time ✅" --> OK["OK & Burn Token 🔥"]
    S -- "2nd Time ❌" --> Fail["Reject"]
```
...
```mermaid
graph TD
    Start["Token"] --> JWKS["1. Fetch JWKS 🔐"]
    JWKS --> Sign["2. Verify Signature ✍️"]
    Sign --> Payload["3. Check Claims (iss, aud, exp) 📝"]
    Payload --> AppID["4. Check App ID 🧿"]
    AppID --> OK["✅ Validated"]
```
