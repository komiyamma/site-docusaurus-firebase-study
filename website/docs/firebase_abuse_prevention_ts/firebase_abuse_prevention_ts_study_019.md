...
flowchart LR
    Task{"タスクの種類"}
    Task -->|調査・実装・計画| A["Antigravity🚀<br>(自律エージェント)"]
    Task -->|リポジトリ監査・相談| G["Gemini CLI💻<br>(ターミナルエージェント)"]
    Task -->|再現環境の共有| S["Firebase Studio☁️<br>(Nixベース環境)"]
```
...
```mermaid
graph LR
    User[ユーザー] -- Request --> Proxy{AI Logic Proxy 🧿}
    Proxy -- "App Check OK ✅" --> RL{Rate Limit 🚦}
    RL -- "100 RPM OK ✅" --> Model[Gemini API]
    Proxy -- "Fail ❌" --> 403[403 Forbidden]
    RL -- "Over ❌" --> 429[429 Too Many Requests]
```
...
```mermaid
graph TD
    M1[Gemini 2.0 Flash] -- "2026-03-31 Retire 🧨" --> M2[Gemini 2.5 Flash]
    M1 --- Notice[AIによる自動棚卸し 🧾]
```
...
```mermaid
graph LR
    FB[Firebase Functions] --> Node[Node 20/22 ✅]
    FB --> Python[Python 3.10-3.13 ✅]
    Node --- Latest[Node 24 Active LTS ⚙️]
```
...
```mermaid
graph TD
    CLI[Gemini CLI] -- "Audit Command" --> Code[Local Codebase]
    Code -- "Findings" --> Report[Vulnerability List 🔎]
    Report -- "Fix" --> Code
```
...
```mermaid
graph TD
    Mission[Mission: Protect AI] --> Plan[1. Plan 📋]
    Plan --> Exec[2. Implement 🛠️]
    Exec --> Verify[3. Verify 🧪]
```
...
```mermaid
graph LR
    Nix[.idx/dev.nix] -- "Define" --> Env[Compute Env]
    Env -- "npm install" --> Deps[Verified Deps]
    Env -- "preview" --> Browser[Shared Preview]
```
