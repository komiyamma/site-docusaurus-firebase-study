...
```mermaid
graph LR
    Mode{"VITE_APPCHECK_MODE"}
    Mode -- off --> Fail[わざと失敗させる💥]
    Mode -- debug --> Debug[Debug Provider 🧪]
    Mode -- recaptcha --> Prod[本番 reCAPTCHA v3 🧿]
```
...
```mermaid
graph TD
    Local[Local Browser] -- Output 🎫 --> Console[Dev Console]
    Console -- Copy & Paste --> FB[Firebase Console]
    FB -- Safelist ✅ --> Access[Access Granted]
```
...
```mermaid
graph LR
    Req[Callable Request] --> Gate{enforceAppCheck: true}
    Gate -- Valid Token --> Handler[Action OK ✅]
    Gate -- No Token --> Reject[401 Unauthorized ❌]
```
...
```mermaid
graph TD
    AI[AI Logic] --> L1[Layer 1: App Check 🧿]
    L1 --> L2[Layer 2: Rate Limit 🚦]
    L2 -- "100 RPM" --> Safe[安心運用 💸]
```
...
```mermaid
flowchart TD
  App["テストアプリ"] --> Mode{"VITE_APPCHECK_MODE"}
  Mode -->|"off"| M1["強制OFF💥<br>(未検証リクエスト)"]
  Mode -->|"recaptcha"| M2["本番モード🧿<br>(reCAPTCHA トークン)"]
  Mode -->|"debug"| M3["デバッグモード🧪<br>(Debug トークン)"]
  
  M1 -->|拒否されるべき| Server{"Firebase サービス<br>(強制ON)"}
  M2 -->|許可されるべき| Server
  M3 -->|許可されるべき| Server
```
...
```mermaid
graph LR
    Anti[Antigravity: Mission 📋] --> Team[AI Solution Team]
    CLI[Gemini CLI: Audit 🔎] --> Team
    Studio[Studio: Env ☁️] --> Team
```
