...
```mermaid
graph TD
    Start["📊 結果画面をチェック"] --> Guard{ガードレール悪化?}
    Guard -- YES --> Stop["🚨 中止 / 撤退"]
    Guard -- NO --> Sig{有意差あり?}
    
    Sig -- "YES (p < 0.05)" --> Winner{プラス差?}
    Winner -- YES --> Win["🏆 採用 (Rollout)"]
    Winner -- NO --> Lose["🧯 却下 (現状維持)"]
    
    Sig -- "NO (p > 0.05)" --> Sample{サンプル十分?}
    Sample -- NO --> Wait["🤔 保留 (継続)"]
    Sample -- YES --> NoDiff["⚖️ どっちでも良い"]
```
