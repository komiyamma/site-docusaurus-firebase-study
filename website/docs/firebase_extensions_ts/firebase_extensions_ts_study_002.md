...
```mermaid
graph TD
    Start["🤔 何を自動化したい?"] --> Search["🔎 Hubで検索"]
    Search -- "機能名で引く" --> Res1["📷 image, email..."]
    Search -- "トリガーで引く" --> Res2["📦 storage, firestore..."]
    
    Res1 --> Detail["📋 詳細ページ確認"]
    Res2 --> Detail
    
    Detail --> Check1["💳 課金プラン (Blaze?)"]
    Detail --> Check2["🔐 必要な権限"]
    Detail --> Check3["🏗️ 作られるリソース"]
    
    Check1 & Check2 & Check3 --> Decision{"これにする?"}
    Decision -- YES --> Install["🚀 導入へ"]
```
