...
```mermaid
graph LR
    Setup["1. セットアップ<br>(init emulators)"] --> Config["2. 拡張登録<br>(ext:install --local)"]
    Config --> Start["3. 起動<br>(emulators:start)"]
    Start --> Test["4. 動作テスト<br>(UI/Script)"]
    Test -- 失敗 --> Config
    Test -- 成功 --> Deploy["🚀 本番デプロイへ"]
```
