...
```mermaid
graph TD
    Start["❓ プランとバケットは?"] --> S1{"Spark (無料) ?"}
    S1 -- NO --> OK["✅ すでにBlaze<br>運用監視へ"]
    S1 -- YES --> S2{"appspot.com か?"}
    
    S2 -- YES --> Alert1["⚠️ 2026/2/3 以降<br>Console利用 & API 停止"]
    S2 -- NO --> Alert2["⚠️ Storage継続不可"]
    
    Alert1 & Alert2 --> Action["🚀 Blazeへアップグレード!"]
```
