...
![Query Filter](./picture/firebase_firestore_base_ts_study_014_01_query_filter.png)

```mermaid
graph TD
    Col[(Todos Collection)] --> Q["query(col, where('done', '==', false))"]
    Q --> R1[Todo A: done=false]
    Q --> R2[Todo B: done=false]
    Q -.-> X[Todo C: done=true]
    style X stroke-dasharray: 5 5, opacity: 0.5
```
