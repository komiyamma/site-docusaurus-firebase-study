...
```mermaid
graph TD
    subgraph MultiRange["複数範囲クエリ (2軸フィルタ)🔥"]
        Q["クエリ開始"] --> F1["salary >= 50000"]
        F1 --> F2["experience >= 5"]
        F2 --> O1["orderBy('salary')"]
        O1 --> O2["orderBy('experience')"]
    end
```
...
flowchart LR
    ...
    A -- "先にorderBy" --> ScanLarge["インデックスのスキャン量が多い 💸"]
    B -- "先にorderBy" --> ScanSmall["インデックスのスキャン量が少ない ⚡"]
    
    ScanSmall -.- Note_ScanSmall["絞り込みが強い方を先に！"]
```
