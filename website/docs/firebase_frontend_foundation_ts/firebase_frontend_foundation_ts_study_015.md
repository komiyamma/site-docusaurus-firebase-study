...
```mermaid
flowchart TD
    subgraph Offset ["Offset 方式 (高コスト)"]
        O1[1-100件をスキップ] -- 裏で読み取っている --> O2[101-110件を返す]
    end
    subgraph Cursor ["Cursor 方式 (低コスト)"]
        C1[100件目のドキュメントを保持] -- そこから先だけ指定 --> C2[101-110件を返す]
    end
```
...
sequenceDiagram
    ...
    participant H as useUsersPaging
    participant F as Firestore

    UI ->> H: loadMore
    Note over H: loading: true
    H ->> F: getDocs (limit + startAfter)
    F -->> H: snap.docs
    H ->> H: 最後のdocをlastVisibleに保存
    H ->> UI: items追加 + loading: false
```
