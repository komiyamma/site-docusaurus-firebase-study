...
```mermaid
graph TD
    Client[フロント/クライアント] -- "秘密情報 / 重い処理" --> Backend[Firebase Functions]
    Backend -- "結果 / 通知" --> Client
```
...
```mermaid
graph TD
    subgraph Trigger ["3つの起動ボタン"]
        H[HTTP トリガー] -- 呼んだら動く --> A[API / Webhook]
        E[イベント トリガー] -- 何か起きたら動く --> B[DB更新 / ファイル追加]
        S[スケジュール] -- 時間が来たら動く --> C[定期バッチ]
    end
```
...
flowchart TD
    Q{何をする処理？}
    Q -- 見た目の制御/即時反応 --> Front[A: フロントエンド]
    Q -- セキュリティ/重い処理/自動化 --> Back[B: Functions]
    Q -- シンプルな読書権限守護 --> Rules[C: Security Rules]
```
...
sequenceDiagram
    ...
    participant F as Functions (Guardrail)
