...
```mermaid
graph LR
    R1[index.ts は薄く] --- R2[フォルダで分類] --- R3[libに共通化]
```
...
```mermaid
flowchart LR
    Cloud[Firebase Cloud] -- 呼び出し --> Index[index.ts]
    Index -- export元を参照 --> H[http/health.ts]
    Index -- export元を参照 --> FS[firestore/onCreated.ts]
```
