...
```mermaid
graph LR
    subgraph Browser ["ブラウザ (Same-Origin Policy)"]
        Site[Webサイト]
    end
    subgraph API ["外部 API (Functions)"]
        Header[CORS許可ヘッダ]
    end
    Site -- リクエスト --> Header
    Header -- "許可リストにあれば通す" --> Site
```
...
```mermaid
graph TD
    O[Origin] --- S[スキーム: https]
    O --- D[ドメイン: example.com]
    O --- P[ポート: :443]
```
...
```mermaid
flowchart LR
    B[ブラウザ] -- 守る --> CORS[CORSの壁]
    C[curl / 外部Bot] -- 守らない --> CORS
    CORS -- 素通り --> API[Functions API]
```
...
```mermaid
graph TD
    L1[Layer 1: CORS] -- ブラウザを限定 --> L2[Layer 2: Auth]
    L2 -- ログイン者を限定 --> L3[Layer 3: App Check]
    L3 -- 正当なアプリを限定 --> Final[安全な本処理]
```
...
```mermaid
flowchart LR
    H[Header] --> A[Authorization: Bearer ...]
    H --> AC[X-Firebase-AppCheck: ...]
    A --> V1[Auth 検証]
    AC --> V2[App Check 検証]
```
