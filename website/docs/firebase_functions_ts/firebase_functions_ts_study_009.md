...
sequenceDiagram
    ...
    participant F as Functions (onCall)

    C ->> F: 呼び出し (Data + Auth Token)
    Note over F: トークンを自動検証
    F ->> F: 実行 (request.auth)
    F -->> C: レスポンス (JSON)
```
...
```mermaid
flowchart TD
    Req[onRequest / Callable] --> Auth{認証済み?}
    Auth -- No --> E401[401 / unauthenticated]
    Auth -- Yes --> AppCheck{App Check?}
    AppCheck -- NG --> E401
    AppCheck -- OK --> Logic[本処理]
```
...
```mermaid
graph TD
    Console[ブラウザコンソール] -- Debug Token --> FB[Firebase Console]
    FB -- 登録済み --> Access[アクセス許可]
```
...
```mermaid
graph LR
    Req[要件] --> AI[AI Assistance]
    AI -- コード提案 --> Impl[実装 / デプロイ]
    Impl --> Test[テスト]
```
