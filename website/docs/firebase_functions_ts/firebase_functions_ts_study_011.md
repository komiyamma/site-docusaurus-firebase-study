...
```mermaid
graph TD
    subgraph Triggers ["Firestore トリガー"]
        C[onDocumentCreated<br/>作成時]
        U[onDocumentUpdated<br/>更新時]
        D[onDocumentDeleted<br/>削除時]
        W[onDocumentWritten<br/>すべて]
    end
```
...
```mermaid
graph TD
    Write[1. Firestore書き込み] --> Trig[2. トリガー発火]
    Trig --> Func[3. Functions実行]
    Func -- "同じ場所に書き戻し" --> Write
    style Write fill:#f99
```
