...
```mermaid
graph LR
    E["Event Source"] -- 配信 --> F["Functions 1回目"]
    E -- 再送 (たまに) --> F["Functions 2回目"]
    F -.- Note_F["同じ処理が2回走る可能性"]
```
...
```mermaid
graph LR
    In["Input"] --> F["Functions"]
    F --> Out["Result A"]
    In --> F
    F --> Out
    Out -.- Note_Out["2回やっても結果は A のまま"]
```
...
```mermaid
graph TD
    P1["入口ガード<br/>早期return"] --> P2["重複排除<br/>ロック作成"]
    P2 --> P3["冪等な副作用<br/>await必須"]
```
