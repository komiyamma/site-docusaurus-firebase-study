...
```mermaid
graph TD
    C["Comment 作成"] --> F["Functions"]
    subgraph Tasks["Functions の仕事"]
        A["Post のカウンタ +1"]
        N["通知ドキュメント作成"]
    end
    F --> A
...
```mermaid
graph TD
    E1["Event 1"] -- 遅延 --> P["Processing"]
    E2["Event 2"] -- 到着 --> P
    E1 -- 再送 --> P
    P -.- Note_P["順序バラバラ & 重複あり"]
```
...
```mermaid
flowchart TD
    Start["Event"] --> Marker{"マーカー作成?"}
    Marker -- 成功 --> Update["カウンタ +1 & 通知作成"]
    Marker -- 失敗 --> Skip["処理済みスキップ"]
```
---
...
```mermaid
graph LR
    F["Functions"] -- 作成 --> D["Notification Doc"]
    D -- リアルタイム購読 --> R["React App"]
    R -- ベルバッジ更新 --> U["User"]
```
...
```mermaid
graph TD
    subgraph Shards["分散カウンタ (Shards)"]
        S1["Shard 0"]
        S2["Shard 1"]
        S3["Shard 2"]
    end
    Shards -- 合計 --> Total["Total Count"]
```
