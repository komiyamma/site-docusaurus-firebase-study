...
```mermaid
graph LR
    Clock["目覚まし時計"] -- 指定時刻 --> F["Functions"]
    F -- 自動実行 --> Biz["レポート作成 / 掃除"]
```
...
```mermaid
graph LR
    UTC["UTC 0:00"] -- "+9h" --> JST["JST 9:00"]
    JST -.- Note_JST["timeZone: #quot;Asia/Tokyo#quot;"]
```
...
```mermaid
flowchart TD
    Day["日付 ID: 2026-04-10"] --> Doc{"create?"}
    Doc -- 成功 --> Write["レポート書き込み"]
    Doc -- 失敗 --> Skip["既存スキップ"]
```
...
```mermaid
graph LR
    D["集計データ"] --> AI["AI (Firebase AI Logic)"]
    AI --> M["要約メッセージ"]
    M --> App["アプリへ表示"]
```
