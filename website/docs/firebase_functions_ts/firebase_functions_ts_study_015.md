...
```mermaid
graph TD
    E1[Error A at Line 10] --> G1[Group 1]
    E2[Error A at Line 10] --> G1
    E3[Error B at Line 20] --> G2[Group 2]
```
...
```mermaid
graph TD
    M[メトリクスアラート📈<br/>しきい値超え]
    L[ログアラート🧾<br/>特定文字/エラー出力]
```
...
```mermaid
graph LR
    Mon[Cloud Monitoring] -- Webhook --> S[Slack #alerts]
```
