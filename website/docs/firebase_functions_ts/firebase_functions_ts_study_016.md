...
graph LR
    subgraph Local["ローカル環境"]
        C["Code / App"] --> E["Firebase Emulator"]
    end
    subgraph Cloud["クラウド本番"]
        P["Production Firebase"]
    end
```
...
```mermaid
graph TD
    UI["Emulator UI"] --- FS["Firestore"]
    UI --- FN["Functions"]
    UI -.- Note_UI["localhost:4000"]
```
...
```mermaid
graph LR
    S["Start"] -- Import --> E["Emulator"]
    E -- Export --> S
    S -.- Note_S["./emulator-data"]
```
...
```mermaid
graph LR
    App["App SDK"] -- connect...Emulator --> E["Local Emulator"]
    App -- (default) --> Cloud["Production"]
```
...
sequenceDiagram
    ...
    UI ->> UI: データ追加
    UI ->> FN: イベント通知
    FN ->> LOG: ログ出力 (logger.info)
    FN ->> UI: 結果反映 (normalizedText)
```
