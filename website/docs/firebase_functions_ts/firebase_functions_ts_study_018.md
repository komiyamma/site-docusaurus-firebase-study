...
```mermaid
graph LR
    P["Pilot / Human"] -- 命令・レビュー --> AI["Copilot / AI"]
    AI -- 提案・下ごしらえ --> P
```
...
```mermaid
graph TD
    S1["npm g install cli"] --> S2["extension install firebase"]
    S2 --> S3["extension settings"]
```
---
...
```mermaid
graph TD
    C1["/firebase:init"] --> D1["初期化支援"]
    C2["/firebase:deploy"] --> D2["デプロイ支援"]
    C3["/crashlytics:connect"] --> D3["エラー分析"]
```
...
```mermaid
graph LR
    G["Gemini CLI"] -- Extension --> M["MCP Server"]
    M --- P["Prompts"]
    M --- T["Tools"]
    M --- R["Resources"]
```
...
```mermaid
graph LR
    AI["LLM Client"] -- MCP Protocol --> S["Firebase MCP Server"]
    S -- Firebase CLI --> Cloud["Firebase Cloud"]
```
...
```mermaid
graph LR
    Init["/firebase:init"] --> AddAI["Add AI Features"]
    AddAI --> Logic["Firebase AI Logic Setup"]
```
...
```mermaid
graph LR
    N["Node.js 20/22"] --- Default
    P["Python 3.13"] --- Support
    D[".NET 8"] --- CloudRun
```
