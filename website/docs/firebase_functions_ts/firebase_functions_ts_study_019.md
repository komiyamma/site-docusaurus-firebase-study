...
```mermaid
graph LR
    F["Functions"] -- POST (JSON) --> S["Slack Webhook URL"]
    S -- 自動投稿 --> C["#channel"]
```
...
```mermaid
graph TD
    S1["Slack App 作成"] --> S2["Webhook 有効化"]
    S2 --> S3["チャンネル追加"]
    S3 --> S4["URL コピー"]
```
...
```mermaid
graph TD
    Def["defineSecret"] --> Opt["secrets: URL"]
    Opt --> Val["URL.value"]
```
---
...
```mermaid
graph LR
    User["人間"] -- "firebase functions:secrets:set" --> CLI["CLI"]
    CLI -- 保存 --> SM["Secret Manager"]
```
...
```mermaid
graph LR
    E[".secret.local"] -- 上書き --> EM["Emulator"]
    E -.- Note_E["Git 除外必須"]
```
...
```mermaid
graph TD
    L["Leak!"] -- .env に書いちゃった --> G["GitHub 公開"]
    G -- 悪用 --> S["Slack 荒らされる"]
    style L fill:#f99
```
