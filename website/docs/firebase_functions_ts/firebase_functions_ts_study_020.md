...
```mermaid
sequenceDiagram
    participant User as Human (Firestore)
    participant FN as Functions
    participant S as Slack

    User ->> User: Document 更新
    User ->> FN: イベント発火
    FN ->> FN: 重複チェック & 差分抽出
    FN ->> S: Webhook 送信
    S -->> User: 通知確認
...
* [ ] Functions は権限が強いので、想定外の書き込みをしてない？🧨![Event Quirks](./picture/firebase_functions_ts_study_020_02_event_quirks.png)

```mermaid
graph TD
    Q1[No Order Guarantee]
    Q2[At-Least-Once Delivery]
    Q3[No Field-Level Trigger]
```
...
```mermaid
erDiagram
    REPORTS ||--o{ DEDUPE : "references via eventId"
    REPORTS {
        string status
        timestamp updatedAt
    }
...
![Idempotent Logic](./picture/firebase_functions_ts_study_020_04_idempotency_logic.png)

```mermaid
flowchart TD
    Start[Event] --> Doc{eventIdで作成?}
    Doc -- 成功 --> Logic[Slack 通知処理]
    Logic -- 失敗 --> Del[Lock 削除]
    Doc -- 失敗 --> Skip[重複スキップ]
```
...
```mermaid
graph LR
    B[Before Data] --- D{Difference?}
    A[After Data] --- D
    D -- YES --> Notify[Notify Slack]
    D -- NO --> End((End))
```
...
```mermaid
graph TD
    subgraph Blocks ["Slack layout"]
        H[Header: 🧾 Report Updated]
        S1[Section: reportId / Change summary]
        S2[Section: Diff details]
        C[Context: eventId]
    end
```
...
```mermaid
graph LR
    N[Node 20/22] --- Current
    O[Node 18] --- Deprecated
```
