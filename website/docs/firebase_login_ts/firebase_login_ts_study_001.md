...
```mermaid
graph LR
    U["User 👤"] -- Credentials --> Auth["Firebase Authentication 🔐"]
    Auth -- "Verified?" --> UID["User ID (uid) ✅"]
```
...
stateDiagram-v2
    [*] --> Loading
    Loading --> Guest: No User
    Loading --> User: Auth OK
    User --> Guest: Logout
...
```mermaid
graph LR
    Raw["auth/invalid-email 💥"] --> AI{"AI Logic 🤖"}
    AI --> Friendly["メールの形が変かも！<br>もう一回見てね🙂"]
```
...
```mermaid
graph LR
    Anti["Antigravity 🚀"] -- "Plan: メモ作成" --> Doc["Blueprint 📄"]
    CLI["Gemini CLI 💻"] -- "Audit: 抜け確認" --> Code["Auth Logic 🛠️"]
```
