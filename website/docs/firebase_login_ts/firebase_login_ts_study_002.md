...
```mermaid
graph TD
    Console[Firebase Console] --> Build[Build メニュー]
    Build --> Auth[Authentication 🔐]
...
```mermaid
graph LR
    P[Email/Password] --> Switch[Enable Switch ⏻]
    Switch --> Save[Save 💾]
```
...
```mermaid
graph TD
    Step1[Google Provider 🌈] --> Step2[Enable ON]
    Step2 --> Step3[Support Email 📮]
    Step3 --> Step4[Save 💾]
```
...
```mermaid
graph TD
    Settings[Auth Settings] --> Domain[Authorized Domains]
    Domain --> D1[localhost ✅]
    Domain --> D2[127.0.0.1 ✅]
    Domain --> D3[your-app.web.app ✅]
```
...
```mermaid
graph LR
    H[Hacker 🕵️‍♂️] -- "Scan Email" --> Guard{Enumeration Protection}
    Guard -- ON --> Safe[模糊なエラー 🛡️]
    Guard -- OFF --> Leak[存在がバレる 💣]
```
...
```mermaid
graph LR
    Btn[Add user 👤] --> Modal[Input: Email/Pass]
    Modal --> Created[User List ✅]
```
...
```mermaid
graph LR
    AI[AI Assistant 🤖] -- Scan --> Config[Console Settings]
    Config -- Audit --> Report[チェックリスト作成 🧾]
```
