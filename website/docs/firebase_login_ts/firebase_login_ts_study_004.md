...
```mermaid
graph TD
    S1["1. まだ分からない⏳<br>(初期化中)"] --> S2["2. ログイン中👤"]
    S1 --> S3["3. ログアウト中🚪"]
```
...
sequenceDiagram
    ...
    participant Obs as onAuthStateChanged
    participant UI as React UI

    SDK ->> Obs: 状態確定 (Login/Logout)
    Obs ->> UI: callback("user")
    UI ->> UI: loading=false, user=user
```
...
```mermaid
graph TD
    Base["App Root"] --> Prov["AuthProvider 🦴"]
    Prov --> Child1["Header"]
    Prov --> Child2["Main Content"]
    Prov --> Child3["AuthGate"]
```
...
graph TD
    Gate{"AuthGate"}
    Gate -- loading=true --> Spinner["認証状態を確認中…⏳"]
    Gate -- loading=false --> User{"user exists?"}
    User -- Yes --> In["SignedIn Area 🎉"]
    User -- No --> Out["SignedOut Area 🔑"]
```
...
```mermaid
graph LR
    State["Auth State"] --> AI{"AI Logic 🤖"}
    AI -- Prompt --> Model["Gemini"]
    Model -- Result --> UX["やさしい説明文 🙂"]
```
