...
```mermaid
graph TD
    subgraph Signup_Screen [サインアップ画面✍️]
        N["ニックネーム"]
        E["メールアドレス"]
        P["パスワード"]
        C["パスワード(確認)"]
...
```mermaid
graph LR
    Code["Firebase Error Code 💥"] --> Map{"authErrorJa"}
    Map -- "email-already-in-use" --> M1["すでに使われています"]
    Map -- "weak-password" --> M2["パスワードが弱いです"]
    Map -- "invalid-email" --> M3["形式が変です"]
```
...
```mermaid
graph TD
    Input["ユーザー入力"] --> State["useState / useMemo"]
    State -- "Validation OK ✅" --> Submit["Submit 🚀"]
    Submit --> SDK["createUserWithEmailAndPassword"]
    SDK -- Success --> Prof["updateProfile ✨"]
    SDK -- Error --> Error["setError 💥"]
```
...
```mermaid
graph TD
    Policy["Password Policy 🛡️"] --> R1["最小 6〜30 文字"]
    Policy --> R2["大文字/小文字 必須"]
    Policy --> R3["数字/記号 必須"]
    Policy --> SDK["validatePassword で事前チェック"]
```
...
```mermaid
graph LR
    Input["Error / Condition"] --> AI{"AI Logic 🤖"}
    AI -- Gemini --> Hint["やさしいヒント文 ✨"]
    Hint --> UI["Signup UI に表示"]
```
...
```mermaid
graph LR
    Prompt["Prompt 📝"] --> CLI["Gemini CLI 💻"]
    CLI -- Generate --> Code["Signup Component 🧱"]
    Code -- Review --> App["Ready to Use!"]
```
