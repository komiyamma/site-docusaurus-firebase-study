...
```mermaid
graph LR
    Signup["サインアップ"] -- "sendEmailVerification" --> Verify["メール確認📩"]
    Forgot["パスワード忘れ"] -- "sendPasswordResetEmail" --> Reset["パスワードリセット🔑"]
```
...
```mermaid
graph TD
    User["Local User Object"] -- "emailVerified: false" --> UI["Verification Banner"]
    Action["メールのリンクを踏む"] -- Server Data Update --> Server["Firebase Auth Server"]
    Reload["user.reload() 🔄"] -- Fetch latest --> Server
    Server -- "emailVerified: true" --> User
    User -- "UI Update" --> Done["Banner Disappears 🎉"]
```
...
```mermaid
graph LR
    Result["createUser... OK ✅"] --> Send["sendEmailVerification 📩"]
    Send --> Msg["「メールを送ったよ」を表示"]
```
...
graph TD
    ...
    User -- true --> Hide["非表示"]
    User -- false --> Show["バナー表示⚠️"]
    Show --> Resend["再送ボタン🔁"]
    Show --> Refresh["確認した！ボタン🔄"]
```
...
```mermaid
graph LR
    Input["Email Input 📧"] --> SDK["sendPasswordResetEmail"]
    SDK -- "HTTPS Request" --> FB["Firebase"]
    FB -- "Send URL" --> Inbox["User's Inbox 📩"]
```
...
```mermaid
graph TD
    Console["Firebase Console"] --> Settings["Auth Settings"]
    Settings --> Domains["Authorized Domains"]
    Domains --> L["localhost ✅"]
    Domains --> P["prod-app.web.app ✅"]
```
...
```mermaid
graph LR
    Code["verify / reset"] --> AI{"AI Logic 🤖"}
    AI -- Gemini --> UX["やさしい説明文 ✨"]
```
