...
```mermaid
graph TD
    P[GoogleAuthProvider] --> SDK[signInWithPopup]
    SDK --> Res{結果}
    Res -- 成功 --> User[User オブジェクト]
    Res -- 失敗 --> Err[Error コード]
```
...
```mermaid
graph LR
    E["auth/popup-blocked 💥"] --> F{Friendly Logic}
    F --> UI["ポップアップがブロックされちゃった💦<br>設定を確認してね！"]
```
...
```mermaid
graph LR
    Err[Google Auth Error] --> AI{AI Logic 🤖}
    AI -- Gemini --> UX[やさしい言い換え文 ✨]
```
...
```mermaid
graph LR
    Old[Traditional Popup] -- "Security & UX" --> FedCM[FedCM / Redirect]
    FedCM -- "Mobile First" --> SDK[Best Practices]
```
