...
sequenceDiagram
    ...
    participant B as ブラウザ (Cookie制限あり)

    App ->> FB: signInWithRedirect
    FB ->> B: 状態保存 (3rd-party Cookie)
    B -->> FB: ブロック！！🚫
    FB -->> App: (戻ってくるが状態が消えている)
...
graph TD
    Start[Hosting環境は何？]
    Start -- "Firebase Hosting (.web.app)" --> Opt1[Option 1: authDomain合わせ]
    Start -- "Firebase Hosting (カスタムドメイン)" --> Opt1
    Start -- "外部Hosting" --> OptX[Option 3〜5 を検討]
    Opt1 -- "もっと簡単にするなら" --> Opt2[Option 2: Popupを優先]
```
...
```mermaid
graph LR
    Btn[ログインボタン] --> Store[sessionStorage にフラグ保存 🚩]
    Store --> SDK[signInWithRedirect]
```
...
```mermaid
graph TD
    Init[アプリ起動 / useEffect] --> Res[getRedirectResult 実行]
    Res -- "result exists ✅" --> Success[ログイン完了]
    Res -- "result is null ❓" --> Flag{フラグある？}
    Flag -- No --> Normal[普通に開いた状態]
    Flag -- Yes --> Alert[Cookie制限で失敗の疑い ⚠️]
```
...
```mermaid
graph TD
    Step1[1. authDomain を今のドメインに変更]
    Step2[2. GCP Console で Redirect URI 追加]
    Step3[3. Firebase Console で Authorized Domain 追加]
    Step1 --> Step2 --> Step3
```
...
```mermaid
graph LR
    Chrome[Chrome 設定] --> Privacy[プライバシーとセキュリティ]
    Privacy --> Cookie[サードパーティ Cookie]
    Cookie --> Block[「すべてブロック」にして検証 🧪]
```
...
```mermaid
graph LR
    State[ホスト名/設定] --> AI{AI Logic 🤖}
    AI -- Gemini --> Hint[「authDomainを直すと動くよ！」✨]
```
