...
graph TD
    User["User (UID: abc) 👤"]
    User --- P1["Email/Pass 🔑"]
    User --- P2["Google Login 🌈"]
    User --- P3["GitHub Login 🐙"]
```
...
```mermaid
graph LR
    Existing["Email/Pass で登録済"]
    Action["後から Google でログイン"]
    Action -- "Email Match" --> Error["account-exists-with-different-credential 💥"]
```
...
graph TD
    subgraph MyPage["マイページ"]
        List["連携済み：<br>- メール/パスワード ✅<br>- Google (未連携) ❌"]
        Btn["Google を連携する 🌈🧷"]
    end
```
...
graph TD
    ...
    Data --- D1["[0]: password"]
    Data --- D2["[1]: google.com"]
    D1 --- Map["linked.includes('password')"]
```
...
sequenceDiagram
    ...
    U ->> A: Googleログイン
    A ->> F: signInWithPopup
    F -->> A: Error (credential保持)
    A ->> U: 「メールで入って連結してね」
    U ->> A: メール/パスログイン
    A ->> F: linkWithCredential
    F -->> A: 連結完了🎉
```
...
```mermaid
graph TD
    User["UID: abc"] -- "Unlink Google" --> User2["UID: abc (No Google)"]
    UAction["再度 Google ログイン"] --> NewUser["新しく UID: xyz が作成される 💥"]
```
...
```mermaid
graph LR
    Conflict["衝突発生"] --> AI{"AI Logic 🤖"}
    AI -- Gemini --> UX["「まずメールで入ってね」のやさしい案内 ✨"]
```
