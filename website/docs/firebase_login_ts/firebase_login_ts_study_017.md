...
graph LR
    ...
    subgraph Firestore [Firestore 🗂️]
        D["プロフィール"]
        E["ニックネーム"]
        F["アプリ設定"]
    end
...
```mermaid
graph TD
    Start["Transaction 開始"] --> Read["snap を取得"]
    Read --> Exist{"Exists?"}
    Exist -- No --> Create["Set (New + common)"]
    Exist -- Yes --> Update["Set (Merge + common)"]
    Update --> End["完了"]
    Create --> End
```
...
```mermaid
graph LR
    Auth["onAuthStateChanged"] -- user --> Check["ensureUserDoc"]
    Check -- Transaction --> DB["(#quot;Firestore#quot;)"]
```
...
```mermaid
graph LR
    Cred["UserCredential"] --> Info["getAdditionalUserInfo"]
    Info -- isNewUser: true --> Welcome["初回限定処理 ✨"]
    Info -- isNewUser: false --> Regular["通常ログイン処理"]
```
...
```mermaid
graph LR
    Req["Request / uid"] -- Compare --> Rule{"Rules: auth.uid == uid"}
    Rule -- Pass --> Data["Read/Write OK"]
    Rule -- Fail --> Deny["Insufficient Permissions"]
```
...
```mermaid
graph LR
    User["新ユーザー"] --> AI{"AI Logic 🤖"}
    AI -- Gemini --> Msg["ようこそ！✨<br>素敵な時間になりますように…"]
```
