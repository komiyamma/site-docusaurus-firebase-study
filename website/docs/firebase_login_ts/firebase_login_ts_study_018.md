...
```mermaid
sequenceDiagram
    participant C as クライアント (React)
    participant S as サーバー (Functions)
    participant A as Admin SDK
    participant F as Firebase Auth

    C ->> C: user.getIdToken()
    C ->> S: Request (Authorization: Bearer <ID Token>)
    S ->> A: verifyIdToken(token)
    A ->> F: 公開鍵で署名検証 🔑
    F -->> A: 署名 OK
    A -->> S: Decoded Token (uid: "abc...")
    S -->> C: JSON Response ✅
```
...
```mermaid
graph TD
    Req[Header 取得] --> Bearer{Bearer ある？}
    Bearer -- No --> E401[401 Unauth]
    Bearer -- Yes --> Verify[verifyIdToken]
    Verify -- "Valid ✅" --> Process[uid 確定 -> ロジック実行]
    Verify -- "Invalid ❌" --> E401
```
...
```mermaid
graph LR
    subgraph Request [onRequest 🛠️]
        R1[cors 設定]
        R2[header パース]
        R3[トークン検証]
    end
    subgraph Callable [onCall ✨]
        C1[自動 CORS]
        C2[検証済み request.auth]
    end
```
...
```mermaid
graph TD
    Node[Node.js 20/22] --- TS[TypeScript]
    Py[Python 3.13] --- AuthLib[firebase-admin]
    DotNet[.NET 8/10] --- Run[Cloud Run functions]
```
...
sequenceDiagram
    ...
    S -->> B: 204 No Content (Allowed headers)
    B ->> S: GET/POST (Authorization: Bearer ...) 🚀
```
...
```mermaid
graph TD
    A[IDトークン] --> B{verifyIdToken}
    B -- "署名/期限 ✅" --> C{失効チェック？}
    C -- "checkRevoked: false" --> O1[検証成功 ✨]
    C -- "checkRevoked: true" --> O2[強制ログアウト検知も可能 🛡️]
```
...
```mermaid
graph LR
    E401[401 Error] --> AI{AI Logic 🤖}
    AI -- Gemini --> UI[「ログインが切れたよ。<br>もう一回入ってね🙏」✨]
```
