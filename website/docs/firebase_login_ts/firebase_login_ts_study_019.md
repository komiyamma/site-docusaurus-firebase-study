...
```mermaid
graph LR
    K["Something you know<br>🧠 Password"] --- Plus("+")
    H["Something you have<br>📱 Phone"] --- Plus
    Plus --> Access["Access Granted 🔓"]
```
...
```mermaid
graph TD
    subgraph A["All Users (Heavy) 🧱"]
        A1["全件 MFA 強制"]
    end
    subgraph B["Optional (Light) 🍃"]
        B1["ユーザーが自分で選ぶ"]
    end
    subgraph C["Step-up (Balanced) ⚖️"]
        C1["重要操作時のみ MFA"]
    end
```
...
sequenceDiagram
    ...
    User ->> App: ログイン (Easy 🙂)
    App ->> User: ログイン成功
    User ->> App: 重要操作 (Critical 🛑)
    App ->> Auth: MFA 要求
    Auth ->> User: SMS/TOTP コード 📱
    User ->> App: コード入力
    App ->> User: 操作許可 ✅
```
...
graph TD
    Check["重要操作リスト"]
    Check --- I1["Change Password 🔑"]
    Check --- I2["Payment 💳"]
    Check --- I3["Delete Data 🗑️"]
    Check --- I4["API Key 🗝️"]
```
...
```mermaid
graph TD
    E["multi-factor-auth-required 💥"] --> R["getMultiFactorResolver"]
    R --> UI["MFA 入力用モーダル表示 ✨"]
```
...
```mermaid
graph LR
    U["Why MFA? 🧐"] --> A{"AI Logic 🤖"}
    A -- Friendly --o Ans["あなたの資産を守るためだよ！💰"]
```
