...
![Route Structure Map](./picture/firebase_login_ts_study_020_01_route_structure.png)

```mermaid
graph TD
    Root["/"] --> Public["Public Area<br>(/login, /signup)"]
    Root --> Guard["RequireAuth 🚧"]
    Guard --> Private["Private Area 🔐<br>(/mypage)"]
```
...
![RequireAuth Logic Flow](./picture/firebase_login_ts_study_020_02_require_auth_logic.png)

```mermaid
graph TD
    Start["アクセス"] --> Loading{"Loading?"}
    Loading -- Yes --> Spinner["Spinner ⏳"]
    Loading -- No --> User{"User?"}
    User -- Yes --> Content["Content ✅"]
...
![Google Login Fallback](./picture/firebase_login_ts_study_020_03_google_login_flow.png)

```mermaid
graph LR
    Attempt["Popup"] -- "Blocked 🚫" --> Fallback["Redirect 🚪"]
    Fallback --> Success["Login"]
```
...
![Login Page Wireframe](./picture/firebase_login_ts_study_020_04_login_page_ui.png)
...
![Redirect Result Retrieval](./picture/firebase_login_ts_study_020_05_redirect_result.png)

```mermaid
graph LR
    Init["App Start"] --> Check["getRedirectResult()"]
    Check -- Found --> User["User Found"]
    User --> Set["Set User State"]
```
...
![AI Error Explanation Flow](./picture/firebase_login_ts_study_020_06_ai_error_flow.png)

```mermaid
graph LR
    Err["Login Failed ❌"] -- Click --> AIBtn["Ask AI 🤖"]
    AIBtn --> Gemini{"Gemini"}
    Gemini --> Sol["Check your caps lock! 💡"]
```
...
![Chapter Completion Checklist](./picture/firebase_login_ts_study_020_07_checklist.png)

```mermaid
graph TD
    subgraph Checklist [最終チェックリスト]
        I1["Routes OK 🧭"]
        I2["Guard OK 🚧"]
        I3["Redirect OK 🔁"]
        I4["AI OK 🤖"]
