...
```mermaid
graph TD
    Client["Client 📱"] -- Trigger --> Func["Cloud Functions ⚡<br/>Trusted Server"]
    Func -- Send --> FCM["FCM Backend 🚀"]
    FCM -- Deliver --> Target["Target Device 📲"]
...
```mermaid
graph LR
    subgraph Support [Node.js Runtimes]
        V22["Node.js 22 🚀<br/>Latest"]
        V20["Node.js 20 ✅<br/>Stable"]
        V18["Node.js 18 ⚠️<br/>Deprecated"]
    end
```
...
flowchart TD
    ...
    Auth -- Fail --> Err["401 Unauthorized"]
    Auth -- OK --> Val{"Validation? ✅"}
    Val -- Fail --> Err2["400 Invalid Arg"]
    Val -- OK --> Send["Admin SDK: send() 📤"]
...
sequenceDiagram
    ...
    B ->> F: httpsCallable("sendTestPush")
    F ->> FCM: messaging.send("msg")
    FCM -->> F: messageId
    F -->> B: { ok: true }
```
...
```mermaid
graph TD
    ADC["ADC / Service Account"] --> Token["Generate OAuth2 Token 🔑"]
    Token --> POST["POST v1/messages:send"]
...
```mermaid
graph LR
    Input["Long Comment 📜"] --> AI{"AI Logic 🤖"}
    AI -- JSON Mode --> JSON["{<br/>&nbsp;&nbsp;title: '...',<br/>&nbsp;&nbsp;body: '...'<br/>} 📦"]
```
