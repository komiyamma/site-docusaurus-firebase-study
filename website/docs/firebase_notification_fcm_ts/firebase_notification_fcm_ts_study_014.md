...
```mermaid
graph LR
    User["User 👤"] -- Write Comment --> DB["(#quot;Firestore 🗃️#quot;)"]
    DB -- event: onDocumentCreated --> Trigger["Cloud Functions ⚡"]
    Trigger -- messaging.send --> FCM["FCM Backend 🚀"]
    FCM -- Push --> Owner["Post Owner 📲"]
```
...
```mermaid
graph TD
    Start["Get UIDs"] --> Check{"targetUid == commenterUid?"}
    Check -- Yes (Self) --> Skip["Skip Notification 🤫"]
    Check -- No (Other) --> Proceed["Fetch Tokens & Send 🔔"]
```
...
```mermaid
graph LR
    Func["Cloud Functions"] -- sendEachForMulticast --> FCM["FCM Service"]
    FCM --> T1["Token A (Phone) 📲"]
    FCM --> T2["Token B (Web) 💻"]
    FCM --> T3["Token C (Tablet) 📟"]
```
...
```mermaid
graph LR
    React["React Application"] -- comment + AI prompt --> AI{"AI Logic 🤖"}
    AI -- short_text --> DB["(#quot;Firestore#quot;)"]
    DB -- trigger --> Function["Functions"]
    Function -- send --> Push["Push Notification"]
```
...
```mermaid
graph TD
    Loop["Start Function"] --> Env{"DRY_RUN == 'true'?"}
    Env -- Yes --> Log["Log payload & Return 📝"]
    Env -- No --> Send["Real API Call 🚀"]
```
...
```mermaid
graph LR
    User["Self Activity"] --X--> Notify["Self Notification 🔕"]
    User --O--> Other["Notify Other Users 🔔"]
```
