...
graph TD
    User["User 👤"]
    User --- D1["Phone 📲<br/>Token A"]
    User --- D2["Laptop 💻<br/>Token B"]
    User --- D3["Tablet 📟<br/>Token C"]
```
...
```mermaid
flowchart TD
    Start["Get All Tokens"] --> Chunk["Divide into batches of 500"]
    Chunk --> Send["sendEachForMulticast"]
    Send --> Result["Check responses order"]
    Result -- Error Found --> Cleanup["Delete invalid tokens from DB 🧹"]
    Cleanup --> Next{"Next Batch?"}
    Next -- Yes --> Chunk
    Next -- No --> End["Finished"]
```
...
```mermaid
graph LR
    Tokens["1200 Tokens"] --> B1["Batch 1: 500"]
    Tokens --> B2["Batch 2: 500"]
    Tokens --> B3["Batch 3: 200"]
```
...
```mermaid
graph LR
    T["Tokens Array"] --> R["Responses Array"]
    T --- T1["Token 0"] --> R1["Response 0: ✅"]
    T --- T2["Token 1"] --> R2["Response 1: ❌"]
    R2 -.- Note_R2["Index matches!<br/>Token 1 is invalid."]
```
...
stateDiagram-v2
    [*] --> New: Token Created
    New --> Active : Registered in DB
    Active --> Stale : Long time inactive
    Active --> Invalid : App uninstalled
    Invalid --> [*]: Deleted from DB 🧹
```
...
graph TD
    FCM["FCM Service"]
    FCM --> S["Success: Reach Phone ✅"]
    FCM --> E["Fail: Token Expired ❌"]
    E --> Log["Only this token fails<br/>The whole batch doesn't stop! 🛡️"]
```
