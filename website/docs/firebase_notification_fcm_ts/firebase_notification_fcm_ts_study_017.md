...
```mermaid
graph TD
    subgraph Health["Health Check Tripod"]
        R["Receive Results 📬"]
        C["Cleanup Invalid 🧹"]
        V["Visualize Logs 📊"]
    end
...
```mermaid
graph LR
    subgraph Array["Array (Bad for Scale)"]
        U1["User"] --- T["'tok1', 'tok2', 'tok3'"]
    end
    subgraph Sub["Subcollection (Recommended)"]
        U2["User"] --> S1["tokenA"]
        U2 --> S2["tokenB"]
        U2 --> S3["tokenC"]
    end
...
```mermaid
flowchart TD
    Send["sendEachForMulticast"] --> Res["Get Per-Token Response"]
    Res --> Loop["For Each Response"]
    Loop --> Success{"Success?"}
    Success -- Yes --> Next["Next"]
    Success -- No --> ErrorCheck{"Unregistered / Invalid?"}
    ErrorCheck -- Yes --> Delete["Delete from DB 🧹"]
    ErrorCheck -- No --> Log["Log Error Code 📝"]
    Delete --> Next
    Log --> Next
```
...
```mermaid
graph TD
    Log["Cloud Logs"] --> E1["Execution: ID_ABC"]
    Log --> E2["Execution: ID_XYZ"]
    E1 --- L1["Step 1"] --- L2["Step 2"]
    E2 --- L3["Step 1"] --- L4["Step 2"]
```
...
graph LR
    Codes["FCM Errors"]
    Codes -- "Status: 404/410" --> Cleanup["Cleanup Needed 🧹<br/>Unregistered / Invalid"]
    Codes -- "Status: 400" --> Bug["Check Payload 🐞<br/>Message too big"]
    Codes -- "Status: 401" --> Auth["Check Auth 🔐<br/>Sender mismatch"]
```
...
```mermaid
graph LR
    New["Day 0: Active ✨"] --> Old["Day 30: Stale? 🧊"]
    Old --> Dead["Day 270: Expired 💀"]
```
...
```mermaid
graph LR
    Logs["Last 100 Logs 📜"] --> AI{"AI Logic 🤖"}
    AI -- Summarize --> Report["Summary: 98% Success<br/>Alert: 'invalid-token' increasing in Web!"]
```
