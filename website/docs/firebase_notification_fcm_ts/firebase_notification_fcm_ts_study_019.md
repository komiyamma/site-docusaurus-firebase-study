...
```mermaid
graph LR
    Comment[New Comment] --> Gate{Gatekeeper 🏯}
    Gate -- High Value --> Send[Deliver 🔔]
    Gate -- Low Value --> Drop[Silent 🤫]
```
...
```mermaid
graph TD
    Trigger[Firestore Trigger] --> Judge[AI Judge 🤖]
    Judge --> Save[Save Decision Log 📝]
    Judge --> Filter{Importance > Threshold?}
    Filter -- Yes --> FCM[FCM Send 🚀]
    Filter -- No --> End[Stop]
```
...
```mermaid
graph TD
    All[All Comments] --> R1[Rule: Self Comment?]
    R1 --> R2[Rule: Notification OFF?]
    R2 --> AI[AI: Importance Check 🤖]
    AI --> Send[Final Push 🔔]
```
...
```mermaid
graph LR
    AI[AI Model] -- "Output Mode: JSON" --> JSON["{<br/>&nbsp;&nbsp;send: true,<br/>&nbsp;&nbsp;importance: 80<br/>}"]
    JSON --> Parse[Code: JSON.parse]
```
...
```mermaid
graph TD
    Try[Parse AI JSON] -- Success --> Use[Use AI Score]
    Try -- Error / Timeout --> Rule[Use Safe Default Rule 🛡️]
    Rule -- "Mention / Reply" --> Yes[Send 🔔]
```
...
```mermaid
graph LR
    Console[Firebase Console] -- Change THRESHOLD 50 -> 70 --> RC[Remote Config]
    RC -- realtime update --> Logic[Sending Logic]
    Logic -- Higher bar --> Fewer[Fewer Notifications 🔇]
```
