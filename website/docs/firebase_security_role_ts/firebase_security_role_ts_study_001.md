...
```mermaid
graph LR
    Client["Web App 🌍<br/>Local SDK"] -- Direct SDK Access --> DB[("Firestore")]
    DB -.-> Rules{"Rules 🛡️<br/>審査🚪"}
    Rules -- Reject --> Deny["Permission Denied 🛑"]
    Rules -- Approve --> Access["Data Access ✅"]
```
...
```mermaid
flowchart TD
    Step1["Loosen Rules 🛠️<br/>Debug Mode"] --> Step2["Deploy (Forgot to close) 📤"]
    Step2 --> Step3["Data Leak (Publicly Accessible) 😱"]
    Step3 --> Step4["Incident Report 💥"]
```
...
graph TD
    Rule["allow read, write: if true"]
    Rule -- "Open to World 🌏" --> Risk["Data Loss / Leak 💥"]
    style Risk fill:#f66,stroke:#333
```
...
graph LR
    Rule["if request.time < ..."]
    Clock["Expired! ⏰"]
    Open["Open 🔓"] -- Time passes --> Hidden["Still Open? 😱"]
    Clock -.-> Hidden
```
...
graph TD
    Rule["if auth != null"]
    A["User A Logged In"] -- Access Allowed --> DataB["User B's Private Data 😱"]
    DataB -.- Note_DataB["Login doesn't mean<br/>Authorization!"]
```
...
graph TD
    ...
    Admin["Server / Admin SDK ✈️"] -- Fly over --> DB[("Firestore")]
```
...
```mermaid
graph LR
    Select["Simulation Type: get/create"] --> Sim["Rules Sim 🧪"]
    Sim --> Result["Request Allowed / Denied 🛑"]
```
