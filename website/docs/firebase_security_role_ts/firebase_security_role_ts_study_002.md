...
```mermaid
graph TD
    Client[Path A: Client 🧑‍💻] -- SDK --> Rules{Rules 🛡️} --> FS[(Firestore)]
    Server[Path B: Server 🖥️] -- Admin SDK --> IAM{IAM 🔐} --> FS
    REST[Path C: REST/RPC 🌐] -- OAuth --> IAM --> FS
```
...
```mermaid
graph LR
    subgraph Untrusted [Untrusted Loop 😈]
        C[Client Device] --> H[Modifiable Code]
    end
    subgraph Trusted [Trusted Zone 😇]
        S[Server / Backend] --> L[Controlled Environment]
    end
    Untrusted --- Defense["Rules (Gate)"]
    Trusted --- Defense2["IAM (Identity)"]
```
...
```mermaid
graph TD
    subgraph Map [Access Map]
        A[Client SDK] --> A1[Auth: User] --> A2[Defense: Rules]
        B[Server SDK] --> B1[Auth: Service Account] --> B2[Defense: IAM]
    end
```
...
```mermaid
graph LR
    S1[Post List] -- Client SDK --> R[Rules]
    S2[Admin Stats] -- Server SDK --> I[IAM]
    R --> DB[(Firestore)]
    I --> DB
```
...
```mermaid
graph TD
    subgraph Vault [Server-side Vault 🔒]
        T1[Aggregation 📊]
        T2[Payment / Logic 💳]
        T3[AI Processes 🤖]
    end
...
```mermaid
graph LR
    Code[Source Code] --> AI{Gemini CLI 🕵️}
    AI -- Inspect --> Report[Access Path Report 📝]
    Report -- Review --> Human[Developer 🧑‍⚖️]
```
...
```mermaid
graph TD
    P1[Hide UI 🙈] -- is NOT --> S[Security 🛡️]
    P2[Server Logic] -- Bypasses --> R[Rules 🧱]
    P1 --> Leak[Data Leak! 💥]
    P2 --> Leak
```
