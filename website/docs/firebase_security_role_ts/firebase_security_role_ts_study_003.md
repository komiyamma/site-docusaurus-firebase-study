...
```mermaid
graph TD
    Console["Console Edit 🖱️"] -- Save --> Cloud["Live Rules"]
    CLI["CLI Deploy 🚀"] -- Overwrite --> Cloud
    Local["Local File"] -- rules --> CLI
    Cloud -.- Note_Cloud["Console changes vanish!<br/>😱"]
```
...
```mermaid
graph LR
    Local["Local rules"] -- Git Commit 📌 --> Git["Repo"]
    Local -- deploy --> Cloud["(#quot;Firebase#quot;)"]
...
```mermaid
graph TD
    Root["Project Root"] --> FJson["firebase.json"]
    Root --> FRules["firestore.rules"]
    Root --> FIdx["firestore.indexes.json"]
```
...
```mermaid
graph LR
    JSON["firebase.json"] -- "rules: 'firestore.rules'" --> File["firestore.rules"]
    JSON -- "indexes: '...'" --> Idx["firestore.indexes.json"]
```
...
graph TD
    Cmd["firebase deploy"]
    Cmd -- "--only firestore:rules" --> R["Rules Only 🛡️"]
    Cmd -- "--only firestore" --> B["Rules + Indexes 📦"]
    Cmd -- "--only firestore:dbId" --> S["Specific DB 🎯"]
```
...
```mermaid
graph TD
    JSON["firebase.json"] --> DB1["Default DB"]
    JSON --> DB2["Ecommerce DB"]
    DB1 --- R1["rules.default.rules"]
    DB2 --- R2["rules.ecommerce.rules"]
```
...
```mermaid
flowchart LR
    Local["Edit Local ✍️"] --> Git["Git Commit 📌"]
    Git --> Test["Test / Simulator 🧪"]
    Test --> Deploy["Deploy CLI 🚀"]
```
