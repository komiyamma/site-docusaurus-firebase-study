...
```mermaid
graph TD
    Root["Root"] --> U["users/{uid}"]
    U --> UC["privateProfile"]
    U --> UN["notes subcollection"]
    Root --> P["publicProfiles/{uid}"]
```
...
```mermaid
graph LR
    Doc["Post Document"] --- Data["{ ownerUid: 'alice' }"]
    Hacker["Hacker Bob"] -- create with --> Data2["{ ownerUid: 'alice' }"]
    Data2 -- "Oops! (If not checked carefully)" --> Doc
```
...
```mermaid
graph TD
    UserA["User A"] -- Logged In --> Rules{"if auth != null"}
    Rules -- Access OK --> DataB["User B's Private Data 😱"]
```
...
```mermaid
graph TD
    Req["Request"] --> Login{"Logged in?"}
    Login -- No --> Deny["Deny 🛑"]
    Login -- Yes --> SameUID{"auth.uid == path.uid?"}
    SameUID -- No --> Deny
    SameUID -- Yes --> Allow["Allow ✅"]
```
...
```mermaid
graph LR
    Auth["Firebase Auth"] -- uid: 123 --> UID["user.uid"]
    UID -- "doc('db', 'users', uid)" --> Path["/users/123"]
```
...
```mermaid
graph LR
    Alice["Alice (uid:A)"] -- "Get /users/B" --> Rules{"isOwner('B')"}
    Rules -- "auth.uid(A) != B" --> Deny["Permission Denied 🛑"]
```
...
```mermaid
graph TD
    subgraph Test_Matrix["Test Matrix"]
        T1["Alice -> Alice's Doc: SUCCESS ✅"]
        T2["Alice -> Bob's Doc: FAIL ❌"]
        T3["Guest -> Alice's Doc: FAIL ❌"]
    end
```
