...
graph TD
    ...
    DB --> D2["{ title: 123 }"]
    DB --> D3["{ admin: true } 😱"]
    DB -.- Note_DB["Schemaless = Anything goes!"]
```
...
```mermaid
graph LR
    V["Validation"] --> H1["hasAll: Required 📌"]
    V --> H2["hasOnly: Allowed Only 🛡️"]
    V --> T["is Type: Data Integrity ✅"]
```
...
```mermaid
graph TD
    Note["AI Note Doc"] --> P["prompt: string"]
    Note --> A[answer: string]
    Note --> O[ownerUid: string]
    Note --> C[createdAt: timestamp]
```
...
```mermaid
graph LR
    Hacker["Hacker"] -- "isAdmin: true" --> Rules{"hasOnly"}
    Rules -- Reject 🛑 --> DB["(#quot;DB#quot;)"]
```
...
```mermaid
graph TD
    F["Validation Functions"] --> R["Required?"]
    F --> A["Allowed?"]
    F --> T["Types?"]
    R -- AND --> Final["Grant Access ✅"]
    A -- AND --> Final
    T -- AND --> Final
```
...
```mermaid
graph LR
    G["Gemini CLI"] -- Generate --> RT["Rules & Tests"]
    RT -- Human Review --> Live["Production"]
```
