...
```mermaid
graph TD
    Client["Client 📱"] -- "Massive String 💣" --> DB[("Firestore")]
    DB -- "Cost / Crash 💥" --> App["App Performance"]
```
...
graph LR
    ...
    S2["Script/URL 💉"]
    S3["Impersonation 🥸"]
    S1 & S2 & S3 --> Risk["Security / UI Risks 💥"]
```
...
```mermaid
graph TD
    W["Rules Weapons 🗡️"] --> S["size: Length 🔢"]
    W --> T["trim: Spaces ✂️"]
    W --> M["matches: Pattern 🔤"]
```
...
```mermaid
graph TD
    P["Profile Fields"] --> DN["displayName: Any, 1-20"]
    P --> H["handle: [a-z0-9_], 3-20"]
    P --> B["bio: max 160"]
```
...
```mermaid
graph LR
    Start["String Input"] --> Type{"Is String?"}
    Type -- Yes --> Trim["trim()"]
    Trim -- "Check" --> Size{"isValidSize?"}
    Size -- Yes --> Pattern{"matches?"}
    Pattern -- Yes --> Success["Validation OK ✅"]
```
...
graph TD
    L1["Layer 1: UI Zod 🏢"]
    L2["Layer 2: Security Rules 🚪"]
    L3["Layer 3: AI Logic/Functions 🤖"]
    L1 --> L2 --> L3
```
