...
```mermaid
graph LR
    Hacker["Hacker 💻"] -- "Update { role: 'admin' }" --> Rules{"Rules Check"}
    Rules -- Reject 🛑 --> DB[("Firestore")]
    Hacker -.- Note_Hacker["Direct SDK Request<br/>(Bypassing UI)"]
```
...
```mermaid
graph TD
    Input["Input Data"] --> WhiteList{"In Allowed List?"}
    WhiteList -- Yes --> Pass["Pass ✅"]
    WhiteList -- No --> Block["Block 🛑"]
```
...
```mermaid
graph LR
    Old["Old Data"] -- diff --> New["New Data"]
    New -- affectedKeys --> Diff["{ modified_keys_only }"]
    Diff -- hasOnly --> Check{"Validation"}
```
...
```mermaid
graph TD
    Doc["User Profile"] --> Edit["Editable: title, bio"]
    Doc --> Lock["Locked: role, createdAt"]
    style Lock fill:#f66,stroke:#333
```
...
```mermaid
graph LR
    subgraph Attempt["Update Attempt"]
        P1["{ bio: 'Hi' } ✅"]
        P2["{ role: 'admin' } ❌"]
    end
    Attempt --> Rules{"affectedKeys"}
    Rules --> Result["Status"]
```
...
```mermaid
graph TD
    Pit1["Update uses keys() instead of affectedKeys()"]
    Pit2["Assuming Rules apply to Admin SDK"]
    Pit1 --> Bug["Existing fields block update 😱"]
    Pit2 --> Leak["Privileged bypass 🚨"]
```
...
```mermaid
graph LR
    AI["AI Drafts Allowlist"] --> Human["Human Review"]
    Human -- "Verify keys" --> Check["✅"]
```
