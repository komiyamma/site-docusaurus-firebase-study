...
```mermaid
graph LR
    subgraph Messy["Messy (Copy-Paste)"]
        M1["allow read: if auth!=null"]
        M2["allow write: if auth!=null"]
        M3["allow read: if auth!=null"]
    end
    subgraph Clean["Clean (Functions)"]
        F1["function isSignedIn()"]
        F2["allow read: if isSignedIn()"]
        F3["allow write: if isSignedIn()"]
    end
```
...
```mermaid
graph TD
    Update1["Module A: Fix ✅"]
    Update2["Module B: Fix ✅"]
    Update3["Module C: Forgot! ❌"]
    Update3 --> SecurityLeak["Security Hole! 😱"]
```
...
```mermaid
graph TD
    Rules["Rules Functions"] --> C1["Stack Depth: max 20"]
    Rules --> C2["No Recursion 🚫"]
    Rules --> C3["Max 10 'let' vars"]
```
...
```mermaid
graph LR
    Code["allow update: if isOwner() && onlyFields(['title'])"]
    Code -- Read as --> Eng["Allow update IF owner AND only title changed"]
```
...
```mermaid
graph LR
    Draft["AI Draft"] --> Human["Human Review"]
    Human --> Tests["Unit Tests ✅"]
```
