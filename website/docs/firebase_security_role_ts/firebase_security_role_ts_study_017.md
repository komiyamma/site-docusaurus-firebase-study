...
```mermaid
graph LR
    subgraph Claims["Custom Claims (Token)"]
        T["Fast / No DB Read"]
        U["Fixed Scope"]
    end
    subgraph DB_Roles["Firestore Roles (Doc)"]
        D["Dynamic / Flexible"]
        C["Costly / Quota Limit"]
    end
```
...
```mermaid
graph TD
    User["User {uid}"] --- RoleDoc["roles/{uid} { admin: true }"]
```
...
```mermaid
graph TD
    Project["projects/{projectId}"] --> Members["members/{uid}"]
```
...
```mermaid
graph LR
    Hacker["Hacker 💻"] -- "Update roles/{me} {admin:true}" --> DB[("Firestore")]
    DB -.- Note_DB["Must be blocked by Rules!"]
```
...
```mermaid
graph TD
    Request --> Rules{"get / exists"}
    Rules -- "$ Reading $" --> DB["(#quot;DB#quot;)"]
```
...
```mermaid
graph TD
    Start["Check isAdmin"] --> LoggedIn{"isSignedIn?"}
    LoggedIn -- Yes --> Exist{"role document exists?"}
    Exist -- Yes --> Data{"admin == true?"}
    Data -- Yes --> OK["Allow ✅"]
```
