...
```mermaid
graph TD
    Read["allow read"] --> Get["get 📄"]
    Read --> List["list 📚"]
    Write["allow write"] --> Create["create ✨"]
    Write --> Update["update ✏️"]
    Write --> Delete["delete 🗑️"]
    style Read fill:#f9f,stroke:#333
...
```mermaid
graph LR
    World["The World 🌍"] -- Access --> Rules{"if true"}
    Rules -- "Welcome! ✅" --> DB[("Firestore")]
    Rules -.- Note_Rules["Security level: 0"]
```
...
```mermaid
graph LR
    Alice["Alice {uid:A}"] -- "Update doc_A {ownerId:B}" --> Rules{"Rules"}
    Rules -- "auth.uid(A) == req.ownerId(B)?" --> Fail["❌ (If only checking req)"]
    Rules -- "Old ownerId('A') == req.ownerId('B')?" --> Block["Block 🛑"]
```
...
```mermaid
graph TD
    User["Web Client 📱"] -- Gate --> Rules{"Rules 🛡️"}
    Rules --> DB[("Firestore")]
    Admin["Admin SDK ✈️"] -- Bypass --> DB
```
...
```mermaid
graph TD
    Console["Console Edit 🖱️"]
    Local["Local File 📄"]
    CLI["firebase deploy 🚀"]
    Local -- Overwrite --> Console
    Console -- Out of Sync --x Local
```
...
```mermaid
graph LR
    Rules["Complex Rules 🧱"] --> AI["AI Detective 🔍"]
    AI -- Findings --> Dev["Developer 🧑‍💻"]
```
