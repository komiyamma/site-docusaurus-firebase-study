...
```mermaid
graph LR
    Red["1. Fail 🔴"] --> Green["2. Pass 🟢"]
    Green --> Refactor["3. Cleanup 🧹"]
    Refactor --> Red
```
...
graph TD
    ...
    JSON -- "port: 8080" --> Emulator["Firestore Emulator"]
```
...
```mermaid
graph TD
    Test["Test Execution"] --> Clear["clearFirestore 🧼"]
    Clear --> Seed["Seed Data 🧪"]
    Seed --> Assert["Check Rules ✅/❌"]
```
...
```mermaid
graph LR
    Start["Start Emulator"] --> Run["Run Tests"]
    Run --> Stop["Auto-stop"]
```
...
```mermaid
graph TD
    Guard["Guard Sleeping 😴"]
    Guard -- No Rules Loaded --> Open["Everything Allowed! ✅"]
    Open -.- Note_Open["Tests pass mistakenly"]
```
...
```mermaid
graph LR
    AI["AI Drafts"] --> Test["Test Green?"]
    Test -- No --> Fix["Human Fix"]
    Test -- Yes --> Pass["Safe ✅"]
```
