...
```mermaid
graph LR
    AI["AI (Drafting / Logic) ✍️"] --- Human["Human (Review / Decisions) 🧑‍⚖️"]
```
...
graph TD
    I1["Collections 🗂️"] --> AI
    I2["Role Definitions 🎭"] --> AI
    I3["Operation Matrix 📝"] --> AI
    I4["Risky Fields ☠️"] --> AI
```
...
```mermaid
graph LR
    P["Role: Expert"] --> R["Context: App Specs"]
    R --> D["Requirement: Rules + Test"]
```
...
```markdown
    - No "if true" leaks?
    - List access restricted?
    - Role fields protected?
    - Admin isolated?
```
*(No standard mermaid checklist yet, using list inside box if needed)*
```mermaid
graph TD
    subgraph Checklist["Human Review"]
        C1["No Leaks?"]
        C2["List Restricted?"]
        C3["Roles Protected?"]
    end
...
```mermaid
graph LR
    Rules["AI Generated Rules"] --> Test["Test Suite"]
    Test -- Fail --> Fix["Refine"]
    Test -- Pass --> Safe["Safe to Deploy ✅"]
```
...
```mermaid
sequenceDiagram
    Admin->>Auth: setCustomClaims
    User->>Auth: getIdToken("forceRefresh:true")
    Auth-->>User: Token with Claims 🎫
```
...
```mermaid
graph TD
    AI["AI Draft"] --> Test["Test Chamber"]
    Test --> Review["Human Approval 🧑‍⚖️"]
    Review -- Yes --> Deploy["firebase deploy 🚀"]
```
