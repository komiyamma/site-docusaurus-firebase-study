...
```mermaid
graph TD
    User["General User"] -- Any Request --> Block["Block 🛑"]
    Admin["Admin"] -- Any Request --> DB["(#quot;Admin Collection#quot;)"]
...
```mermaid
graph LR
    AI["AI: Rules + Tests Draft"] --> Human["Human: Review & Refine"]
    Human --> Deploy["Deploy"]
```
...
graph TD
    ...
    Request --> F
    F -- "auth!=null & token.admin==true" --> Allow["Allow"]
    F -- else --> Deny["Deny"]
```
...
```mermaid
graph LR
    JSON["firebase.json"] -- "rules: 'firestore.rules'" --> RulesFile["firestore.rules"]
```
...
```mermaid
graph TD
    Ctx1["unauthenticatedContext</br>Guest"]
    Ctx2["authenticatedContext</br>User"]
    Ctx3["authenticatedContext</br>Admin (admin:true)"]
    Ctx1 & Ctx2 & Ctx3 --> TestRunner["Test Execution"]
```
...
```mermaid
graph LR
    Exec["emulators:exec"] --> Run["npm test"]
    Run --> Stop["Auto Shutdown"]
    Run -- Results --> Output["Pass/Fail"]
```
...
```mermaid
graph TD
    Local["Local Rules (Tested)"] -- "firebase deploy --only firestore:rules" --> Cloud["Firestore Rules ☁️"]
```
