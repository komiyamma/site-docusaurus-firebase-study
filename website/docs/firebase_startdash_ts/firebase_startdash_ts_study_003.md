...
```mermaid
graph TD
    Console["Firebase Console"] --> Top["1. Top Dropdown<br/>(Project Switch)"]
    Console --> Gear["2. Gear Icon<br/>(Project Settings)"]
    Console --> Left["3. Left Menu<br/>(Features: Auth/DB...)"]
```
...
```mermaid
graph LR
    User["Before you click anything!"] --> Check{"Which Project?"}
    Check -- Dev --> Safe["myapp-dev ✅"]
    Check -- Prod --> Warning["myapp-prod ⚠️<br/>Be careful!"]
```
...
```mermaid
graph TD
    Gear["⚙️ (Gear Icon)"] --> GS["Project Settings"]
    Gear --> U["Usage and Billing"]
    Gear --> UAM["Users and Permissions"]
```
...
```mermaid
graph LR
    Settings["General Settings"] --> Scroll["Scroll down to Apps"]
    Scroll --> App["Select Web App"]
    App --> Code["Copy 'firebaseConfig'"]
    Code --> Paste["Paste into React code"]
```
...
```mermaid
graph TD
    Plan["Blaze Plan (Pay as you go)"] --> Free["Free Tier usage"]
    Free --> Over["If exceeded -> Pay"]
    Plan --> Alert["Set Budget Alerts 🔔"]
```
...
```mermaid
graph LR
    Lost["I need my API key!"] --> AskAI["Ask Antigravity/Gemini"]
    AskAI --> Reply["'Go to Gear ⚙️ -> Project Settings -> General -> Web Apps'"]
```
...
```mermaid
graph TD
    H["Treasure Hunt"] --> Q1["Find config object"]
    H --> Q2["Find Firestore rules tab"]
    H --> Q3["Find App Check setup"]
```
