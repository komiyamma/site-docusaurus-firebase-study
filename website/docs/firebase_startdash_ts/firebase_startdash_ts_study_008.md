...
```mermaid
graph LR
    Cmd[npm create vite@latest] --> Init[Project Folder]
    Init --> Deps[package.json + vite.config.ts]
...
```mermaid
graph TD
    Run[npm run dev] --> Server[Vite Dev Server ⚡]
    Server --> Browser[localhost:5173 🌐]
    Browser --> Count[Vite + React UI]
```
...
```mermaid
graph TD
    HTML[index.html] -- script_src --> Main[src/main.tsx]
    Main -- Renders --> App[src/App.tsx]
    App -- UI --> Browser[Browser View]
```
...
```mermaid
graph LR
    App[App.tsx] --> UI[Placeholder UI 🔲]
    UI -- Future --> Logic[Auth / DB Integration 🔌]
```
...
```mermaid
graph TD
    Issue[Command Not Found] --> Cause1[Node.js not installed?]
    Cause1 --> Fix1[Install / Restart Terminal]
    Issue --> Cause2[Wrong folder?]
    Cause2 --> Fix2[cd react-startdash]
```
