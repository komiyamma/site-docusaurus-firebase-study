...
```mermaid
graph TD
    Root["Project Root"] --> Config["vite.config.ts / package.json"]
    Root --> Src["src/ <br/>React Code"]
...
graph LR
    ...
    Pkg --> Scripts["scripts: { dev, build, lint }"]
```
...
```mermaid
graph LR
    Public["public/favicon.ico"] -- Build --> Dist["dist/favicon.ico"]
    Public -.- Note_Public["No Vite processing, just copied"]
```
...
```mermaid
graph TD
    App["App.tsx"] --> Layout["Header/Footer"]
    App --> Feature["Chat Box"]
...
```mermaid
graph TD
    Lib["src/lib/"] --> FB["firebase.ts"]
    Lib --> Util["utils.ts"]
    Lib --> AI["ai.ts"]
    style Lib fill:#f9f,stroke:#333
```
...
```mermaid
graph LR
    IDE["Workspace Context 🧠"] --> Prompt["AI Prompt"]
    Prompt --> Gen["Code Generation 🤖"]
```
