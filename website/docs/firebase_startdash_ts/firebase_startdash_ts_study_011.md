...
```mermaid
graph LR
    P["Firebase Project 📦"] --> N["Project Name<br/>'My App' (Human readable)"]
    P --> ID["Project ID<br/>'myapp-12345' (Globally unique URL base)"]
    P --> Num["Project Number<br/>'0123456789' (For APIs)"]
```
...
```mermaid
graph TD
    Project["Create Project"] --> A{"Analytics?"}
    A -- Dev/Test --> No["OFF 🛑<br/>Simpler, faster"]
    A -- Prod --> Yes["ON ✅<br/>Tracking users"]
```
...
```mermaid
graph TD
    Console["Firebase Console 🏠"] --> Top["Top Dropdown"]
    Console --> Main["Main Screen"]
    Top --> Add1["➕ Add Project"]
    Main --> Add2["➕ Create a Project"]
```
...
```mermaid
graph LR
    User["Types 'chat-app'"] --> Check{"Available?"}
    Check -- No --> Firebase["Adds -xxxx suffix"]
    Firebase --> Final["chat-app-9c8f1"]
```
...
```mermaid
graph TD
    Ask["Ask Gemini"] --> Q["'Give me 5 unique names for a recipe app'"]
    Q --> A["recipe-hub-app, cook-base..."]
```
...
graph LR
    ...
    CLI -- Auth --> Cloud["Google Cloud Auth"]
```
