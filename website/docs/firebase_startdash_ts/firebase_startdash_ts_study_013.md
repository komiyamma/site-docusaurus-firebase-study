...
```mermaid
graph LR
    Install["1. npm install firebase"] --> Init["2. Init in src/lib/firebase.ts"]
    Init --> Verify["3. Verify in src/App.tsx"]
```
...
```mermaid
graph TD
    User["Terminal"] -- node -v --> Node{"Version >= 18?"}
    Node -- Yes --> Proceed["OK ✅"]
    Node -- No --> Update["Update Node 🛑"]
```
...
```mermaid
graph LR
    Cmd["npm install firebase"] --> Save["node_modules/ & package.json"]
```
...
```mermaid
graph TD
    Config["firebaseConfig"] --> Init["initializeApp"]
    Init --> App["Firebase App object"]
```
...
```mermaid
graph TD
    App["App.tsx"] --> Import["import app from lib/firebase"]
    Import --> UI{"app.name?"}
    UI -- exists --> Success["Show: 'Connected!'"]
```
...
```mermaid
graph TD
    FileA["File A: initializeApp"] --> App1["App Created"]
    FileB["File B: initializeApp"] --> Error["FirebaseError: duplicate-app 💥"]
```
...
```mermaid
graph LR
    IDE["Firebase Configured 🚀"] --> AI["Ask AI: #quot;Add Auth setup#quot;"]
    AI --> Logic["Implement feature"]
```
