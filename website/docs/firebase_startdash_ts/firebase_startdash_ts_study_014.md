...
```mermaid
graph LR
    Browser[Network Tab / JS files 🌐] -- Can read --> Config[Firebase Config Options 🔑]
    Config -- Cannot prevent --> Exposure[Secrets leaked if included! 😱]
```
...
graph TD
    subgraph Config[Config Types]
        OK[firebaseConfig<br/>Public identifiers ✅]
        NG[Server Keys / AppCheck Secret<br/>Strictly Private ❌]
    end
```
...
```mermaid
graph TD
    Dev[.env.development] --> Build[Build Process]
    Prod[.env.production] --> Build
    Build --> Output[Switch config automatically]
```
...
```mermaid
graph LR
    Secret[SECRET_KEY] -- Vite build --> Ignore[Discarded 🗑️]
    Pub[VITE_API_KEY] -- Vite build --> Include[Embedded in JS 📦]
```
...
```mermaid
graph LR
    User[Copy values] --> Env[.env.local]
    Env --> ViteConfig[import.meta.env.VITE_...]
```
...
```mermaid
graph TD
    File[.env.local] -- git commit --> Gitignore{.gitignore Check}
    Gitignore -- Matches *.local --> Block[Not Committed 🛑]
    File2[App.tsx] -- git commit --> Gitignore
    Gitignore -- Pass --> Repo[(Git Repo ✅)]
```
...
```mermaid
graph TD
    Team[New Dev] -- pull repo --> Example[.env.example]
    Example -- cp .env.example .env.local --> Local[.env.local]
    Local -- fill in values --> IDE[Ready to code 💻]
```
