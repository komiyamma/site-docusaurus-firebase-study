...
```mermaid
graph LR
    IDE[AI IDE<br/>Cursor/Windsurf] -- Requests data --> MCP[MCP Server]
    MCP -- API Call --> Firebase[Firebase ☁️]
    Firebase -- Data (Docs/Stats) --> MCP
    MCP -- Feeds context --> IDE
```
...
```mermaid
graph TD
    Login[firebase login] --> Auth[Firebase Authentication]
    Auth -- Generates token --> Local[~/.config/configstore/]
    Local -- Used by --> MCP[Antigravity MCP]
```
...
```mermaid
graph LR
    Cursor[Cursor Settings] --> Features[Features Tab]
    Features --> MCP[MCP Config]
    MCP --> Add[+ Add New MCP Server]
    Add --> Config[Set command: npx antigravity / ...]
```
...
```mermaid
graph LR
    Setup[Initialize Antigravity] --> Tools[List Available Tools]
    Tools --> Fire[Firebase Info]
    Tools --> GCP[GCP Access]
```
...
```mermaid
graph TD
    Chat[AI Chat] -- /firebase_status --> Agent[Check Env]
    Agent -- Reads --> State[Project State]
    State --> Res[Report: 'All OK']
```
...
```mermaid
graph TD
    Req[Prompt: 'Delete all users'] --> MCP{Antigravity Gate}
    MCP -- Has /Confirm policy --> Rule[Requires explicit 'yes']
    Rule -- Blocks if unsafe --> Halt[Action Stopped 🛑]
```
...
```mermaid
graph LR
    User[Ask AI: 'Current Project?'] --> MCP[MCP / firebase use]
    MCP --> Data[myapp-dev-123]
    Data --> User
```
