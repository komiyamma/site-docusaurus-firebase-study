...
```mermaid
graph TD
    CLI["Gemini CLI"] --> Ext["Firebase Extension Context 🧰"]
    Ext --> P1["Local Firebase JSON"]
    Ext --> P2["Cloud Project State"]
    Ext --> P3["Official Docs Knowledge"]
```
...
```mermaid
graph LR
    Menu["/firebase:..."] --> Init["/firebase:init 🏗️"]
    Menu --> AppCheck["/firebase:appcheck 🛡️"]
    Menu --> Deploy["/firebase:deploy 🚀"]
```
...
```mermaid
graph LR
    Log["Copy Error Log 📋"] --> Agent["AI: #quot;Fix this error#quot;"]
    Agent --> Plan["Analysis & Command Plan"]
    Plan --> Exec["You review & run ✅"]
```
...
```mermaid
graph TD
    Fail["Firebase Failed"] --> E1["Missing-App: Init failed"]
    Fail --> E2["Permissions: Rules block"]
    Fail --> E3["Quota: Limits exceeded"]
```
...
```mermaid
graph LR
    App["App.tsx / ai.ts 🏠"] --> Logic["Firebase AI Setup 🧠"]
    Logic -- Agent builds --> Config["Functions / Secrets Config ⚙️"]
```
...
```mermaid
graph TD
    subgraph Antigravity [Antigravity/Cursor 🛸]
        A["Auto-edit Code 🤖"]
        B["Multi-file changes"]
    end
    subgraph Gemini [Gemini CLI 💻]
        C["Terminal First"]
        D["Cloud Ops/State check"]
    end
```
