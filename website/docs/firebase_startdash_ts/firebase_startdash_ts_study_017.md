...
graph TD
    subgraph Env["Environments"]
        Dev["Project: 'myapp-dev' 🧪<br/>Fake Data / Messy"]
        Prod["Project: 'myapp-prod' 🚀<br/>Real Data / Clean"]
        Dev -. "Strict Border" .-> Prod
    end
```
...
```mermaid
graph LR
    DevCmd["npm run dev"] -- "mode: development" --> Vite1[".env.local / .env"]
    ProdCmd["npm run build"] -- "mode: production" --> Vite2[".env.production / .env"]
```
...
```mermaid
graph TD
    App["App.tsx renders"] --> Env{"import.meta.env.MODE"}
    Env -- 'development' --> DevUI["Display 'DEV MODE 🚧' Banner"]
    Env -- 'production' --> ProdUI["Normal UI ✅"]
```
...
```mermaid
graph LR
    Add["firebase use --add"] --> Conf[".firebaserc"]
    Conf -- "dev: myapp-dev" --> Alias
    Conf -- "prod: myapp-prod" --> Alias
    Cmd["firebase use dev"] -- Loads context --> Proj["myapp-dev"]
```
...
```mermaid
graph TD
    DB["Firestore DB"] --> User["Users Collection"]
    Rule["Security Rules"] --> Config
    AI["AI Keys/Quotas"] --> Config
    DB -.- Note_DB["Scoped tight to ONE Project"]
```
...
```mermaid
graph TD
    Prod["Target: PROD 🚀"] --> Code["Deploy Rules"]
    Code -- Oh wait! --> Check{"firebaseAuth.uid == realAdmin?"}
