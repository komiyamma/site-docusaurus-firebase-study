...
```mermaid
graph LR
    Browser["App (React)"] -- Call / Trigger --> Backend["Firebase Project ☁️"]
    Backend --> Auth["Auth"]
    Backend --> DB["(#quot;Firestore#quot;)"]
    Backend --> Func["Functions"]
```
...
```mermaid
graph TD
    Node18["Node 18: Older, approaching EOL ⚠️"]
    Node20["Node 20: Recommended LTS ✅"]
    Node22["Node 22: Latest LTS 🌱"]
    style Node20 fill:#bfb,stroke:#333,stroke-width:2px
```
...
graph TD
    ...
    Firebase["Firebase SDK"] -- Wraps --> NodeJS
```
...
```mermaid
graph LR
    Local["node -v (e.g. v20.x.x)"] -- Matches? --> Config["package.json: 'engines: { node: \#quot;20\#quot; }'"]
    Config -- Deploys to --> Cloud["Cloud Node.js 20 Env ☁️"]
```
...
```mermaid
graph TD
    Goal["Add AI Feature"] --> Easy["App Vertex SDK 🟢"]
    Goal --> Robust["Cloud Functions + API 🔵"]
    Goal --> Quick["Firebase/Gemini Extensions 🟡"]
    Easy --> Client["Browser calls AI"]
    Robust --> Server["Hidden Server Logic"]
```
...
```mermaid
graph TD
    subgraph AI Assistant Tier
        Console["Console AI💬"]
        CLI["Gemini CLI💻"]
        MCP["Cursor/IDE MCP🛸"]
    end
    subgraph Backend Capability
        SDK["Vertex SDK for Firebase"]
        Func["Genkit / Functions"]
    end
```
