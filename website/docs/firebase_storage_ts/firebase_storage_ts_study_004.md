...
```mermaid
graph TD
    Ref["Storage Reference 📍<br/>Pointer to cloud location"]
    Ref -- "ref(storage, 'path/file.jpg')" --> Cloud["Cloud Storage ✨"]
    Ref -- Holds --> Data["Path, Name, Bucket info"]
```
...
```mermaid
graph LR
    Root["gs://bucket-name/"] --> Users["users/"]
    Users --> U1["uid-123/"]
    U1 --> Img["avatar.png"]
```
...
```mermaid
graph TD
    File["Original: 'photo.jpg'"] --> UUID{"Use UUID?"}
    UUID -- No --> Over["Old 'photo.jpg' overwritten ❌"]
    UUID -- Yes --> Safe["'5f3a...-photo.jpg' ✅"]
```
...
```mermaid
graph LR
    Invalid["Path: 'users/../hacked'"] -- Rules Block --> Reject["Error 🛑"]
    Valid["Path: 'users/uid456/file'"] -- Rules Pass --> Allowed["Success ✅"]
```
...
```mermaid
graph TD
    SDK["import { ref, getStorage } from 'firebase/storage'"] --> GetStr["const storage = getStorage()"]
    GetStr --> BuildRef["const fileRef = ref(storage, 'path/' + filename)"]
```
...
```mermaid
graph LR
    Ref["Storage Path 'docs/doc123.pdf'"] --> Ext["Firebase Extension Context"]
    Ext --> Agent["AI Agent"]
    Agent --> Ans["Summarizes doc123.pdf"]
```
