...
```mermaid
graph LR
    File["Large 500MB Video"] --> Task["uploadBytesResumable()"]
    Task -- Chunk 1 --> Cloud["Storage"]
    Task -- Chunk 2 --> Cloud
    Task -. Paused .-> Cloud
    Task -- Resumed --> Cloud
```
...
```mermaid
graph TD
    Upload["Upload Progress"] --> Trans["bytesTransferred"]
    Upload --> Total["totalBytes"]
    Trans --> Calc["Math: Trans / Total * 100"]
    Calc --> UI["UI: 45%"]
```
...
```mermaid
graph TD
    Start["task.on"] --> Run["state: 'running'"]
    Run --> Prog["snapshot: progress UI"]
    Run --> Pause["state: 'paused'"]
    Prog --> Success["Next: upload complete"]
    Prog --> Error["Error handling"]
```
...
```mermaid
graph LR
    Agent["MCP Agent"] -- Prompt --> Code["Tailwind Progress Bar Code"]
    Code -- Adjust --> Smooth["Add transition-all duration-300 ✨"]
```
