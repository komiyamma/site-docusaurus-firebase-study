...
```mermaid
graph TD
    User["User Clicks Delete"] --> API["deleteObject()"]
    API --> Cloud["Storage"]
    Cloud -- Gone Forever --> Ouch["Data Lost 😱"]
```
...
```mermaid
graph TD
    Data["Image Data"] --> Hard["Hard Delete<br/>Instant loss"]
    Data --> Soft["Soft Delete<br/>Flag 'deleted'"]
    Data --> Grace["Grace Period<br/>Keep for 30 days"]
```
...
```mermaid
graph LR
    Active["status: 'active'"] -- User deletes --> Pending["status: 'pending_delete'"]
    Pending -- Background job --> Removed["Firestore Doc Deleted"]
```
...
```mermaid
graph TD
    Cloud["Firebase Cloud Functions"] --> Trigger["Trigger: every 24h"]
    Trigger --> Query["Query 'pending_delete' older than 30d"]
    Query --> DelObj["storage.deleteObject()"]
```
...
```mermaid
graph TD
    Cron["Cloud Scheduler Timer"] --> Func["onSchedule('24h')"]
    Func --> Log["Batch delete 100 items... 🧹"]
```
...
```mermaid
graph LR
    Dev["Need logic to find old docs"] --> Gem["Ask MCP: Write query"]
    Gem --> Code["where('deletedAt', '<', 30_days_ago)"]
```
...
```mermaid
graph TD
    Task1["Add status field to DB"] --> Task2["Create logical delete UI"]
    Task2 --> Task3["Hide 'pending_delete' items"]
```
