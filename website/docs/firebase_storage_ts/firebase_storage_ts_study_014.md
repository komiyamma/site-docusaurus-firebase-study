...
```mermaid
graph TD
    Rule["Security Rules Protect Path"] --> Block["Block unauthorized access 🛑"]
    URL["Download URL"] --> Public["Publicly accessible if token known ⚠️"]
    Rule -.- Note_Rule["URL bypasses rules!"]
```
...
graph LR
    subgraph Bad Practice
        URL["Store full https://... token"]
    end
    subgraph Good Practice
        Path["Store 'users/123/avatar.jpg'"]
        Path -- At runtime --> Fetch["Fetch short-lived URL or blob"]
    end
```
...
```mermaid
graph TD
    React["Load Component"] --> Fetch["getDownloadURL()"]
    Fetch -- Pending --> Skeleton["Show Skeleton UI ⏳"]
    Fetch -- Success --> Show["Show Image 🖼️"]
    Fetch -- Fail --> Fallback["Show Default Avatar 👤"]
```
...
```mermaid
graph LR
    Client["Browser"] -- "getBlob(ref)" --> Storage["Firebase Storage 🪣"]
    Storage -- Blob Object --> Client
    Client -- URL.createObjectURL() --> IMG["<img src='blob:...' />"]
```
...
```mermaid
graph TD
    Err["Image Not Displaying 💔"] --> A["CORS Error?"]
    Err --> B["403 Forbidden? <br/>Check Rules"]
    Err --> C["404 Not Found? <br/>Check Path"]
```
...
```mermaid
graph LR
    Dev["Copy failed fetch code"] --> Agent["AI Agent 🤖"]
    Agent --> Fix["Suggest try/catch and fallback return"]
```
...
```mermaid
graph TD
    T1["Implement Async Image Component"] --> T2["Handle Loading State"]
    T2 --> T3["Add Error Fallback"]
```
